...
| AIチャットパネル | 1920px | 200px（デフォルト、100-400px調整可） | y: 880 | AI支援機能 |
...
> **Monaco Editor上書き実装**: Ctrl+クリックはMonaco既定で「Go to Definition」に割当。選択モードON時はこの既定動作を上書きし、非連続選択として動作させる。実装時は`editor.addCommand`でCtrl+Clickをインターセプトし、選択モードフラグに応じて動作を分岐させる。
...
>
> **※1**: [AI]ボタンクリックでAIチャットパネルの表示/非表示を切り替え。AIチャットパネルは画面下部に固定表示され、上端のリサイザーで高さ調整可能（デフォルト200px、調整範囲100-400px）。
>
...
| D-8 | GUIパネルを3セクション構成（S1参加者/S2要素一覧/S3スタイル）とし、S2でメッセージ・フラグメント・ノートを統合管理 | 簡素化、認知負荷低減、関係性の一目把握 |
