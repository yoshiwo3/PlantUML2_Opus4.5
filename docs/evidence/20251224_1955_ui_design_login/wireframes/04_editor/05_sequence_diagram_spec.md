# エディタ画面（シーケンス図）機能仕様書 v3.11

**作成日時**: 2026-01-01
**作成者**: Claude Code
**対象画面**: エディタ画面（シーケンス図編集）
**対応UC**: UC 3-1〜3-6, 4-1, 4-2
**対応TD**: TD-017〜TD-035, D-1〜D-70, D-101〜D-103
...
8. [[#8. メッセージ入力UI|メッセージ入力UI]]
    ...
    - [[#8.5 UI設計（600px幅）|8.5 UI設計（600px幅）]]
...
9. [[#9. フラグメント入力UI|フラグメント入力UI]]
    ...
    - [[#9.4 UI設計（600px幅）|9.4 UI設計（600px幅）]]
...
10. [[#10. ノート入力UI|ノート入力UI]]
    ...
    - [[#10.4 600pxで実現可能（D-18〜D-22）|10.4 600pxで実現可能]]
    - [[#10.5 UI設計（600px幅）|10.5 UI設計（600px幅）]]
...
12. [[#12. 画面遷移図|画面遷移図]]
    ...
    - [[#12.4 S1参加者追加フロー|12.4 S1参加者追加フロー]]
...
| UC 3-2 | テンプレート選択 | シーケンス図テンプレート適用 |
...
```
┌─────────────────────────────────────────────────────────────────────────────────────────────────┐
│ 04_editor / 01_default_v2 - エディタ画面 (基本状態)                                             │
│ Ref: TD-015 (低精度WF), TD-006 (Storage設計) | パネル幅 GUI 600 + Code 520 + Preview 800 = 1920px│
├─────────────────────────────────────────────────────────────────────────────────────────────────┤
│ PlantUML Studio │ sequence_diagram.puml │ ● 保存済     [保存][エクスポート] [GUI+Code][GUI][Code][AI]│
├──────────────────────────────┬──────────────────────────┬────────────────────────────────────────┤
│ GUI パネル              [○]  │ Code パネル    [🔍][↔️][○]│ Preview パネル  [−][100%][+][Fit][1:1]│
├──────────────────────────────┼──────────────────────────┤                                        │
│ ▼ S1: 参加者 (3)   [+ 参加者]│ 1  @startuml             │                                        │
│ ┌──────────────────────────┐ │ 2  participant User      │  ┌─────────────────────────────────┐   │
│ │ [A] User      [↑↓✎🗑]   │ │ 3  participant Server    │  │                                 │   │
│ │ [P] AuthService [↑↓✎🗑] │ │                          │  │                                 │   │
│ │ [D] Database  [↑↓✎🗑]   │ │                          │  │   【シーケンス図プレビュー】    │   │
│ └──────────────────────────┘ │                          │  │                                 │   │
├──────────────────────────────┤                          │  │                                 │   │
...
│ [スタイル詳細設定]           │                          │                                        │
├──────────────────────────────┴──────────────────────────┴────────────────────────────────────────┤
│ AI Chat パネル │ Claude 3.5 Sonnet ▼                    [学習モード][▶ 適用][再生成][○] ▼       │
│ AIとの対話履歴がここに表示されます...                                                            │
│ ┌───────────────────────────────────────────────────────────────────────────────────────[送信]─┐ │
│ │ 💬 メッセージを入力...                                                                       │ │
│ └──────────────────────────────────────────────────────────────────────────────────────────────┘ │
...
| GUIパネル | 600px | 832px | x: 0, y: 48 | S1/S2/S3セクション |
| Codeパネル | 520px | 832px | x: 600, y: 48 | Monaco Editor |
| Previewパネル | 800px | 832px | x: 1120, y: 48 | SVGプレビュー |
| AIチャットパネル | 1920px | 200px | y: 880 | AI支援機能 |

**サイズ検証**:
- 横: 600 + 520 + 800 = **1920px** ✓
- 縦: 48 + 832 + 200 = **1080px** ✓
...
|-----------|------|--------|---------|------|
| GUI-Code間 | x: 596-604 | 8×832px | GUIパネル幅 | min:200px, max:800px |
...
| 操作 | 結果 |
|------|------|
| いずれかのトグルをON | **全3パネルが同時にON** |
| いずれかのトグルをOFF | **全3パネルが同時にOFF** |

#### 各パネルでの選択方法

| パネル | 選択方法 | 複数選択 | 選択対象 |
|--------|---------|:--------:|---------|
| **GUIパネル** | メッセージカードをクリック | 累積選択可（複数回クリック） | 修正対象の要素を指定 |
| **Codeパネル** | ドラッグで範囲選択 | 連続範囲のみ | 修正対象のコード行を指定 |
| **AIチャット** | ドラッグでテキスト範囲選択 | 連続範囲のみ | AI回答から適用コードを選択 |
...
```
トグルON → AI回答内のコードをドラッグして範囲選択 → 適用コードが選択状態
         → [▶ 適用]ボタンがアクティブ化
...
|------|----------|-----------|--------------|
| GUIで要素選択 | 選択カード表示 | 黄色ハイライト | メッセージ色変更 |
| Codeで範囲選択 | 対応カードハイライト | 選択範囲表示 | 対応メッセージ色変更 |

> **注意**: GUI/Code併用は排他（後から選択した方が有効）
...
| モード | ショートカット | パネル構成 | GUIパネル幅 | 用途 |
|--------|:-------------:|-----------|:-----------:|------|
| **Mode 1（デフォルト）** | Ctrl+1 | GUI + Code + Preview | 600px | 標準編集 |
| **Mode 2** | Ctrl+2 | GUI(拡大) + Preview | 950px | GUI詳細編集 |
| **Mode 3** | Ctrl+3 | Code(拡大) + Preview | - | コード直接編集 |

### 3.2 Layer構造

| レイヤー | 説明 | 用途 |
|---------|------|------|
| **Layer 1 (Mode 1)** | GUIパネル 600px | 閲覧・標準操作 |
| **Layer 1 (Mode 2)** | GUIパネル 950px | インライン編集・D&D |
| **Layer 2** | モーダル | 複雑な編集（フラグメント作成、Group/Activate設定） |

### 3.3 操作権限マトリクス

| 操作               | Mode 1 (600px) | Mode 2 (950px) | Layer 2 (モーダル) |
| ---------------- | :------------: | :------------: | :------------: |
| 一覧閲覧             |       ✅        |       ✅        |       -        |
| 選択（プレビュー連動）      |       ✅        |       ✅        |       -        |
| 削除               |       ✅        |       ✅        |       ✅        |
| メッセージ追加          |   → Layer 2    |     ✅ クイック     |       ✅        |
| メッセージ編集          |    → Mode 2    |    ✅ インライン     |       ✅        |
| メッセージ並び替え        |       ❌        |     ✅ D&D      |       ✅        |
| フラグメント追加         |   → Layer 2    |   → Layer 2    |       ✅        |
| フラグメント編集（単純型）    |    → Mode 2    |    ✅ 条件ラベル     |       ✅        |
| フラグメント編集（複合型）    |   → Layer 2    |   → Layer 2    |       ✅        |
| ノート追加            |   → Layer 2    |     ✅ クイック     |       ✅        |
| Group/Activate追加 |   → Layer 2    |   → Layer 2    |       ✅        |
...
| # | セクション | 内容 |
|:-:|-----------|------|
| S1 | 参加者 | Actor/Participant/Database等 |
| S2 | メッセージ | 矢印メッセージ |
...
| # | セクション | 内容 | 高さ |
|:-:|-----------|------|-----:|
| **S1** | **参加者** | Actor / Participant / Database / Entity 等 | 200px |
...
### 4.2 セクション高さ（Mode 1: 600px幅時）

| 領域 | 高さ | Y位置 | 内容 |
|------|-----:|------:|------|
| パネルヘッダー | 48px | 48 | "GUI パネル" + 選択モードトグル |
| S1セクション | 200px | 96 | 参加者管理 |
...
> **関連用語**: [[#参加者（Participant）|→ 参加者]] | [[#ライフライン（Lifeline）|→ ライフライン]]

### 5.1 概要

シーケンス図に登場する参加者（Actor, Participant, Database, Entity等）を管理するセクション。
...
| タイプ | アイコン | 説明 | PlantUML構文 | 用途例 |
|--------|:--------:|------|-------------|--------|
| actor | 👤 | 人間のユーザー | `actor Name` | エンドユーザー、管理者 |
| participant | □ | システム/コンポーネント | `participant Name` | サービス、モジュール |
| boundary | ⊏ | 境界（UI/API等） | `boundary Name` | 画面、APIエンドポイント |
| control | ○ | 制御（ビジネスロジック） | `control Name` | コントローラー、ユースケース |
| entity | ◇ | エンティティ（データ） | `entity Name` | ドメインオブジェクト |
| database | 🗄️ | データベース | `database Name` | RDB、NoSQL |
| collections | ⊞ | コレクション | `collections Name` | リスト、配列 |
| queue | ⎔ | キュー | `queue Name` | メッセージキュー |

> **ECB（Entity-Control-Boundary）パターン**: boundary/control/entityはロバストネス図で使用される3要素。システム設計時の責務分離に有効。

### 5.4 UI設計（Mode 1: 600px幅）
...
| 操作 | 動作 | 備考 |
|------|------|------|
| **☰ ドラッグ** | 参加者の順序変更 | PlantUMLの参加者宣言順に反映 |
| **[✏] クリック** | 編集フォームをインライン展開 | §5.6参照 |
...
| 操作 | 動作 | 展開方式 |
|------|------|:--------:|
| **[+ 参加者を追加]** クリック | 参加者追加フォームが展開 | インライン |
...
| 3 | 名前重複 | 追加クリック時 | 「同じ名前の参加者が既に存在します」 |
...
**順序変更時の動作**:
1. S1リスト内のカード順序を変更
2. Codeパネルの参加者宣言順を更新（PlantUMLの表示順に影響）
3. Previewパネルが自動更新（参加者の左右位置が変化）

> **PlantUML仕様**: 参加者の宣言順が図内の左→右の表示順に対応
...
| **activate** | 外部左縦線（縦位置揃え）+ **参加者ラベル縦書き** | PlantUML図と同じ表現、一直線 |
| **メッセージ** | カードUI | 編集・削除ボタン付き |
| **フラグメント** | ラッパー + 条件ラベル | alt/opt/loop等を視覚的に囲む |
| **ノート** | バッジ表示 | メッセージカード付近に表示 |
...
| 幅 | 6px |
| 色 | #388e3c（緑） |
| 参加者ラベル | 縦書きで縦線に沿って表示（例: S-e-r-v-e-r） |
| クリック領域 | 幅12px以上確保（タッチ操作対応） |
| ホバー時 | 縦線ハイライト + ツールチップ（参加者名）+ cursor: pointer |
| クリック | 編集モーダル起動 |
...
| 要素 | 表記 | 説明 |
|------|------|------|
| D&Dハンドル | ☰ | ドラッグ操作の開始点（Mode 2のみ） |
| フラグメント境界 | ┌─ ... ─┐ | 条件分岐等を視覚的に囲む |
| 編集ボタン | [✏] | インライン編集を開始 |
| 削除ボタン | [🗑] | 要素を削除 |
| 詳細編集 | [編集 →] | Layer 2モーダルへ遷移 |
...
| └ **─────────** | セパレーター | - | - |
| └ **区切り線** 選択 | 区切り線をS2末尾に追加 | 即時追加 | §6.10 |
| └ **スペース** 選択 | スペースをS2末尾に追加 | 即時追加 | §6.10 |
| └ **遅延** 選択 | 遅延をS2末尾に追加 | 即時追加 | §6.10 |
| └ **─────────** | セパレーター（範囲選択が必要な要素） | - | - |
| └ **group** 選択 | Group/Activate作成モーダル（groupタブ） | モーダル | §11 |
| └ **アクティブバー** 選択 | Group/Activate作成モーダル（activateタブ） | モーダル | §11 |
| └ **create** 選択 | create追加UIが展開 | インライン | §6.11 |
| └ **destroy** 選択 | destroy追加UIが展開 | インライン | §6.11 |
...
| Mode | D&Dハンドル | 操作方法 |
|:----:|:-----------:|---------|
| Mode 1 (600px) | 非表示 | [⋮]メニュー → 並べ替え |
| Mode 2 (950px) | ☰ 表示 | ☰をドラッグして移動 |

#### カード種別ごとのD&D可否

| カード種別 | D&D移動 | 制約 |
|-----------|:-------:|------|
| **メッセージ** | ✅ | シーケンス内任意位置（フラグメント境界は跨げない） |
| **ノート** | ✅ | 親メッセージの上下のみ |
| **フラグメント** | ❌ | 内部構造が複雑なためLayer 2で対応 |
| **区切り要素** | ✅ | シーケンス内任意位置 |

#### D&D操作フロー

```
1. カード左端の ☰ ハンドルをドラッグ開始
2. ドロップ可能位置にインジケータ（────）表示
3. ドロップで位置確定
4. PlantUMLコード自動再構成
5. Previewが自動更新
```
...
| 制約 | 説明 |
|------|------|
| フラグメント境界を跨ぐ移動 | ❌ 不可（Layer 2モーダルで対応） |
| フラグメント内↔外の移動 | ❌ 不可（構造破壊防止） |
| フラグメント自体の移動 | ❌ 不可（内部構造が複雑） |

> **D-68**: フラグメントのD&D移動は不可（Layer 2で対応）
> **D-69**: D&Dでフラグメント境界を跨ぐ移動は不可
...
区切り要素はシンプルなため、フォーム展開なしで即時追加。

```
[+ 構造要素 ▼] → 区切り線 選択時:
┌─────────────────────────────────────────────────────┐
│ ⓘ 区切り線が追加されました                          │  ← インライントースト
│ ───── 新しいフェーズ ─────                          │  ← S2末尾に追加
│                                              [✏][🗑] │
└─────────────────────────────────────────────────────┘
```
...
### 6.10 参加者ライフサイクル（create/destroy）
...
| タイプ | PlantUML構文 | 視覚 | 用途 |
|--------|-------------|:----:|------|
| create | `create Bob`<br>`Alice -> Bob: new` | ◇→ | 参加者の動的生成 |
| destroy | `destroy Bob` | ×─ | 参加者の破棄 |
...
| # | ルール | エラーメッセージ |
|:-:|--------|-----------------|
| 1 | create対象はS1に存在する参加者 | 「参加者を選択してください」 |
| 2 | create後にメッセージが必要 | 「生成メッセージを入力してください」 |
| 3 | destroy対象はS1に存在する参加者 | 「参加者を選択してください」 |
| 4 | 既にdestroyされた参加者は選択不可 | 「既に破棄された参加者です」 |
...
シーケンス図の**見た目・表示設定**を制御するセクション。S1（参加者）・S2（要素一覧）が「何を描くか」を定義するのに対し、S3は「どう見せるか」を定義する。
...
シーケンス図において、参加者（Actor/Participant等）のライフライン（縦の破線）の**最下部に表示されるボックス**のこと。図の上部にある参加者ボックスと対になり、ライフラインの終端を示す。
...
参加者間を結ぶ矢印の上に表示される**メッセージラベル（テキスト）の水平位置**を指定する設定。矢印の左寄せ・中央・右寄せを選択できる。
...
S3スタイルセクションはシンプルな入力項目のみで構成されるため、600px幅で十分に操作可能。
...
|------|------|-------------|:------:|
| 参加者スタイル | 参加者ボックスの背景色・枠線色・枠線太さをカスタマイズ。Actor/Participant/Database等のタイプ別設定も可能 | `skinparam participant` | 中 |
...
クリック後:
┌────────────────────────────────────────────────────┐
│ ▼ S2: 要素一覧 (6) [+ メッセージ][+ 構造要素 ▼]   │  ← ヘッダーボタン
├────────────────────────────────────────────────────┤
│ 5. Bob → Alice: done                          [🗑] │
│ ┌────────────────────────────────────────────────┐ │
│ │ ⋮⋮ メッセージ追加                             │ │  ← フォーム展開（リストの最後に挿入）
│ ├────────────────────────────────────────────────┤ │
│ │ 送信元: [Alice ▼]                             │ │
│ │ 矢印:   [→同期|-->|<--|↺]                     │ │
│ │ 送信先: [Bob ▼]                               │ │
...
| 項目 | Mode 1 (600px) | Mode 2 (950px) |
|------|---------------|----------------|
| フォーム幅 | 560px | 910px |
| レイアウト | 縦積み | 横並び可 |
| D&Dハンドル | 非表示 | ⋮⋮ 表示（フォーム移動可） |
| 入力後の編集 | [+ メッセージ]で再展開 | インライン編集可 |

### 8.2 入力項目（D-53〜D-58）

| # | 項目 | UI要素 | 必須 | デフォルト | 備考 |
|:-:|------|--------|:----:|-----------|------|
| 1 | 送信元 | ドロップダウン | ✅ | 最後に使用した参加者 | S1参加者リストから選択 |
| 2 | 送信先 | ドロップダウン | ✅ | - | S1参加者リストから選択 |
...
| ↺ 自己 | `->` (同一参加者) | 自己参照メッセージ | ⟳ | 内部処理、再帰呼び出し |
...
|-----------|------------------|-----------|
| → 同期 | 両方表示 | 送信元・送信先必須（別参加者） |
| --> 非同期 | 両方表示 | 送信元・送信先必須（別参加者） |
| <-- 戻り | 両方表示 | 送信元・送信先必須（別参加者） |
| **← return** | **両方非表示** | **メッセージのみ（送信元/送信先は自動決定）** |
| **↺ 自己** | **「対象」に統合** | **対象参加者必須（送信元=送信先）** |
...
| 3 | 送信元≠送信先（通常メッセージ） | 送信先選択時 | 「同じ参加者は選択できません」 |
| 4 | メッセージ必須 | 追加クリック時 | 「メッセージを入力してください」 |
| 5 | S1に参加者が2名以上存在 | フォーム展開時 | 「先に参加者を2名以上追加してください」 |
...
#### 8.6.3 opt/loop/break（単純条件型）入力項目
...
|--------|------|-------------|:----:|------|
| note | over | `note over A` | □ | 参加者の上に配置（単一/複数） |
| note | left of | `note left of A` | □ | 参加者の左側に配置 |
| note | right of | `note right of A` | □ | 参加者の右側に配置 |
...
参加者の上に配置されるノート。複数参加者を跨ぐ場合に適している。
...
#### 10.1.2 left of（参加者左側）

参加者の左側に配置されるノート。補足説明や状態表示に使用。
...
#### 10.1.3 right of（参加者右側）

参加者の右側に配置されるノート。補足説明や状態表示に使用。
...
### 10.4 600pxで実現可能（D-18〜D-22）

ノートはシンプルな入力項目で構成されるため、600px幅で十分操作可能。
...
> **対象参加者について**: 開始メッセージのTo（受信者）で自動決定。別途ドロップダウンは不要（タブ名で表示）。
...
| # | エラー名 | 条件 | 深刻度 | 表示例 |
|:-:|---------|------|:------:|--------|
| A1 | **同一参加者・完全同一** | 同participant, 同range | ❌ | 「Server(2)と完全重複」 |
| A2 | **同一参加者・部分重複** | 同participant, 範囲交差 | ❌ | 「Server(2)と部分重複（不可）」 |
| A3 | **同一参加者・ネスト** | 同participant, 包含関係 | ⚠️ | 「Server(2)内にネスト（稀）」 |
| A4 | **同一参加者・連続** | 同participant, 隣接範囲 | ⚠️ | 「Server(2)と連続（統合推奨）」 |
| A5 | **異参加者・重複** | 異participant, 範囲重複 | ✅ | （エラーなし、正常） |
...
| 参加者ラベル | 縦書き | 縦線に沿って表示（例: S-e-r-v-e-r） |
...
同一参加者の複数activate（ネスト）は、左カラム内で縦線を並列表示：
...
| **D-33** | アクティブバーは同一参加者重複を禁止 | PlantUML制約 |
| **D-34** | 選択タイミングでリアルタイム検証 | 即座にフィードバック |
| **D-35** | group部分重複は警告（適用可） | PlantUMLでは有効 |
| **D-36** | アクティブバー同一参加者部分重複はエラー | 構文エラーになる |
...
│   │   Mode 1          │             │   Mode 2          │              │
│   │   (GUI+Code)      │ ←────────── │   (GUI拡大)       │              │
│   │   GUI: 600px      │   Ctrl+1    │   GUI: 950px      │              │
│   │   Code: 520px     │             │   Code: なし      │              │
│   │   Preview: 800px  │             │   Preview: 970px  │              │
│   │                   │             │                   │              │
│   └─────────┬─────────┘             └─────────┬─────────┘              │
│             │                                 │                         │
│             │ Ctrl+3                          │ Ctrl+3                  │
...
│  ┌─────────────────┐                      ┌─────────────────┐               │
│  │ S2: 要素一覧    │                      │ S2: 要素一覧    │               │
│  │ (Mode 1: 600px) │    [+ 構造要素]      │ (Mode 2: 950px) │               │
│  ├─────────────────┤    クリック          ├─────────────────┤               │
│  │ メッセージ一覧  │ ────────────────────→│ インライン編集  │               │
│  │ 削除のみ       │                      │ D&D並び替え     │               │
│  │                │                      │ クイック追加    │               │
...
| トリガー | 遷移先 | 展開方式 | 入力項目 | 完了後 |
|---------|--------|:--------:|---------|--------|
| **S1: [+ 参加者を追加]** | 参加者追加フォーム | インライン | タイプ、名前、表示名、エイリアス | S1リスト更新 |
| **S2: [+ メッセージ]** | メッセージ追加フォーム | インライン | 送信元、矢印タイプ、送信先、メッセージ、hidden | S2リスト更新 |
| **S2: [+ 構造要素] → フラグメント** | フラグメント追加フォーム | インライン | タイプ、条件ラベル（動的）、内包メッセージ | S2リスト更新 |
| **S2: [+ 構造要素] → ノート** | ノート追加フォーム | インライン | 位置、対象、内容、背景色 | S2リスト更新 |
| **S2: [+ 構造要素] → group** | Group/Activateモーダル | **モーダル** | ラベル、対象範囲（既存メッセージから選択） | S2リスト更新 |
| **S2: [+ 構造要素] → アクティブバー** | Group/Activateモーダル | **モーダル** | 対象参加者、対象範囲（既存メッセージから選択） | S2リスト更新 |
...
│                                    └─ [保存][キャンセル]                     │
│                                                                              │
│  [区切り要素カード] ───[✏]クリック───→ 区切り要素編集フォーム（§6.9.5）    │
│                                        ├─ ラベル編集（区切り線/遅延）         │
│                                        ├─ 高さ変更（スペース）               │
│                                        └─ [保存][キャンセル]                 │
│                                                                              │
...
|------|:------:|:------:|:------:|:--------------:|
| **参加者** | インライン（§5.4） | インライン（§5.6） | ダイアログ（§5.7） | §5 |
...
| **区切り線** | 即時追加（§6.9） | インライン（§6.9.5） | ダイアログ | §6.9 |
| **スペース** | 即時追加（§6.9） | インライン（§6.9.5） | ダイアログ | §6.9 |
| **遅延** | 即時追加（§6.9） | インライン（§6.9.5） | ダイアログ | §6.9 |
| **create** | インライン（§6.10） | インライン | ダイアログ | §6.10 |
| **destroy** | インライン（§6.10） | インライン | ダイアログ | §6.10 |
...
| GUIカード1つ選択 | 有効 | 選択カード情報を`selectedTarget`に設定 |
| GUIカード複数選択 | 有効 | 全選択カード情報を`selectedTarget`に設定 |
| Codeパネル範囲選択 | 有効 | 選択行情報を`selectedTarget`に設定 |
| GUI/Code両方選択 | 有効 | **後から選択した方**を優先 |
...
GUIパネルで複数カードを選択した場合の`selectedTarget`構成。
...
const selectedTarget: SelectedTarget = {
  ...
  items: [
    ...
    { text: "Bob→Server: query", lineNumber: 7, elementType: "message" },
    { text: "Server→DB: select", lineNumber: 9, elementType: "message" }
  ]
};
```
...
2. Bob→Server: query（7行目）[message]
3. Server→DB: select（9行目）[message]
```
...
■ 指示
上記の依頼意図に基づき、すべての修正対象を適用コードで修正してください。
...
| `selectedCode` | AIチャットパネルで選択した適用コード | AI回答から選択した範囲 |
...
| ID | 決定 | 理由 |
|:--:|------|------|
| D-1 | 単純型の条件ラベルはMode 2で編集可 | UX効率向上 |
| D-2 | 複合型の条件ラベルはLayer 2必須 | 構造変更を伴う |
| D-3 | 「非表示」機能は廃止 | シンプルさ優先、削除のみに統一 |
| D-4 | 内包メッセージの編集操作は全てLayer 2 | 一貫性・安全性 |
| D-5 | APIでisSimpleTypeフラグを返却 | フロント判定不要化 |
| D-6 | 深いネスト時は警告バッジ表示 | 編集漏れ防止 |
| D-7 | S2/S3/S4を統合し「シーケンス」セクションとする | UX改善 |
| D-8 | 5セクション→3セクション構成に変更 | 簡素化、認知負荷低減 |
...
### 14.3 D-47〜D-58, D-101〜D-103: 表示・メッセージ設計

> **注**: D-71〜D-100は[[08_section_input_analysis]]で使用済みのため、方式I関連はD-101以降を採番
...
| TD-017 | エディタ画面GUIパネルの図表タイプ別動的切り替え | S2/S3/S4切り替え | §2, §3 |
...
| TD-032 | AIチャットパネル折りたたみ | チャットUI展開/折りたたみ | §9 |
| TD-033 | キーボードショートカットヘルプ | ショートカット一覧表示 | §10 |
...
シーケンス図に登場するオブジェクト、システム、またはアクター。PlantUMLでは `participant`、`actor`、`database` 等のキーワードで定義。
...
**使用箇所**: [[#5. S1: 参加者セクション|§5 S1: 参加者セクション]]

---

#### ライフライン（Lifeline）

参加者から下方に伸びる縦の点線または実線。オブジェクトの生存期間（存在している時間軸）を表現する。
...
**使用箇所**: [[#5. S1: 参加者セクション|§5]], [[#11. Group/アクティブバー入力UI|§11]]
...
ライフラインの下端に表示される参加者名ボックス。シーケンス図の終端で参加者を再度識別しやすくする。PlantUMLでは `hide footbox` で非表示にできる。
...
**使用箇所**: [[#5.5.4 バリデーションルール|§5.5.4]], [[#9.8 バリデーションルール|§9.8]], [[#8.6 バリデーションルール|§8.6]], [[#13.2 バリデーションルール|§13.2]]
...
|-----------|-----------|:------------:|--------------|
| §5 S1参加者 | ParticipantModule | Lv.1 | Lv.1→Lv.1（維持）|
...
| 2025-12-31 | v2.0 | FB10反映: S1参加者追加フォーム詳細化（§5.5）- インライン展開UI、バリデーション、生成コード。S2メッセージ追加フォーム詳細化（§11）- インライン展開UI、Mode差異、ボタン動作、生成コード |
...
| 2025-12-31 | v2.3 | FB12反映: §12画面遷移図大幅拡充。§12.3要素作成フロー修正（フラグメント:モーダル→インライン）。§12.4 S1参加者追加フロー追加。§12.5入力画面遷移マトリクス追加（全6入力画面の遷移・入力項目・完了後動作を一覧化）。§12.6インライン展開vsモーダル判定基準追加。目次更新 |
...
| 2026-01-01 | v2.6 | [[04_screen_spec_default]] v2.3整合性反映: S2ボタン配置修正（フッター→ヘッダー3箇所）、方式H詳細追加（参加者ラベル縦書き）、TD-030参照先修正（フラグメント→ズーム）、§2.6 Previewズームコントロール追加（25%単位、25%-400%範囲、キーボードショートカット） |
...
| 2026-01-01 | v3.11 | 入力・編集画面網羅性改善: §5参加者（boundary/control追加、§5.6編集フォーム、§5.7削除確認、§5.8順序変更）、§8メッセージ（§8.3 return矢印追加、§8.9編集フォーム、§8.10削除確認）、§9フラグメント（§9.10編集フォーム、§9.11削除確認）、§10ノート（hnote/rnote追加、§10.6編集フォーム、§10.7削除確認、§10.8生成コード）、§6 S2（§6.9区切り要素、§6.10 create/destroy）、§12画面遷移（§12.7要素編集フロー、§12.8要素削除フロー、§12.9入力・編集・削除マトリクス）。目次全面更新 |
