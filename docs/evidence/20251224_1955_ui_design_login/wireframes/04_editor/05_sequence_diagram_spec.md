# エディタ画面（シーケンス図）機能仕様書 v3.44

**作成日時**: 2026-01-01
**作成者**: Claude Code
**対象画面**: エディタ画面（シーケンス図編集）
**対応UC**: UC 3-1〜3-6, 4-1, 4-2
**対応TD**: TD-017〜TD-038, D-1〜D-70, D-101〜D-109
...
6. [[#6. S2: 要素一覧セクション|S2: 要素一覧セクション]]
    - [[#6.1 概要|6.1 概要]]
    - [[#6.2 S2要素タイプ|6.2 S2要素タイプ]]
    - [[#6.5 S2要素一覧 UI（統合版）|6.5 S2要素一覧 UI（統合版）]]
    - [[#6.6 UI要素説明|6.6 UI要素説明]]
    - [[#6.7 操作→動作マッピング（D-59〜D-63）|6.7 操作→動作マッピング]]
    - [[#6.8 D&D並べ替え仕様（D-64〜D-70, TD-037）|6.8 D&D並べ替え仕様]]
    - [[#6.9 区切り要素（区切り線/スペース/遅延）|6.9 区切り要素]]
    - [[#6.10 オブジェクトライフサイクル（create/destroy）|6.10 create/destroy]]
    - [[#6.11 S2詳細: メッセージ入力UI|6.11 S2詳細: メッセージ入力UI]]
    - [[#6.12 S2詳細: フラグメント入力UI|6.12 S2詳細: フラグメント入力UI]]
    - [[#6.13 S2詳細: ノート入力UI|6.13 S2詳細: ノート入力UI]]
    - [[#6.14 S2詳細: Group/アクティブバー入力UI|6.14 S2詳細: Group/アクティブバー入力UI]]
...
| AIチャット上端      | y: 876-884   | **1320px**×8 | AIチャット高さ | min:100px, max:1032px |
...
| 操作 | 動作 | 展開方式 |
|------|------|:--------:|
| **ヘッダーの [+]** クリック | オブジェクト追加フォームが展開 | インライン |

> **重要**: [[04_screen_spec_default]] §4.3.1により、S1フッターにはボタンを配置しない（ヘッダーのみ）
...
S2で扱う要素は以下の3カテゴリに分類される。

#### メッセージ

オブジェクト間のやりとりを表現する。矢印タイプでメッセージの性質を区別。

##### 基本タイプ（これだけ覚えればOK）

| S2表示 | 矢印 | PlantUML | 説明 | 用途例 |
|:------:|:----:|----------|------|--------|
| `1.→` | **→** | `->` | 同期メッセージ（実線） | 通常の呼び出し、処理依頼 |
| `2.←` | **<--** | `<--` | 戻りメッセージ（破線） | 明示的な応答、結果返却 |

> **💡 ほとんどの図は `→`（呼び出し）と `<--`（応答）で描けます**

##### 応用タイプ（必要に応じて）

| S2表示 | 矢印 | PlantUML | 説明 | 用途例 |
|:------:|:----:|----------|------|--------|
| `1.⤍` | **-->** | `-->` | 非同期メッセージ（破線） | イベント通知、バックグラウンド処理 |
| `1.↺` | **↺** | `-> self` | 自己参照 | 内部処理、再帰呼び出し |
| `2.⏎` | **⏎** | `return` | 戻り＋終了 | アクティブバー終了と戻り値を同時に表現 |
...
> **💡 ほとんどの図は `→` と `<--` で描けます**
>
> **💡 `return`はアクティブバー使用時に便利**: [[#11.10 生成されるPlantUMLコード|→ §11.10 複数アクティブバー適用例]]
>
> **詳細**: [[#8. メッセージ入力UI|→ §8 メッセージ入力UI]]

#### 構造要素（フラグメント）

複数のメッセージをグループ化し、条件分岐・繰り返し・並列処理を表現。

##### 基本タイプ（これだけ覚えればOK）

| S2表示 | タイプ | 説明 | 用途例 |
|:------:|--------|------|--------|
| 🔀 | **alt** | 条件分岐（if-else） | 認証成功/失敗、在庫あり/なし |
| 🔀 | **opt** | オプション（if only） | エラー時のみ実行 |
| 🔀 | **loop** | 繰り返し | 再試行、バッチ処理 |
...
> **💡 ほとんどの図は alt（条件分岐）、opt（条件付き）、loop（繰り返し）で描けます**

##### 応用タイプ（必要に応じて）

| S2表示 | タイプ          | 説明     | 用途例             |
| :--: | ------------ | ------ | --------------- |
|  🔀  | **break**    | 中断・脱出  | 例外発生時の早期終了      |
|  🔀  | **par**      | 並列処理   | 同時実行、マルチスレッド    |
|  🔀  | **critical** | 排他制御   | トランザクション、ロック区間  |
|  🔀  | **group**    | グルーピング | 論理的なまとまり、フェーズ分け |
...
' ===== critical: 排他制御（他の処理をブロック） =====
critical 決済処理中
  レジ -> 決済システム: 決済実行
  決済システム --> レジ: 決済完了
end

' ===== group: グルーピング（まとまりを示す） =====
group group（締め処理）
  店員 -> レジ: 精算開始
  レジ -> 売上DB: 日計集計
end
...
| S2表示 | タイプ         | 説明          | 用途例          |
| :--: | ----------- | ----------- | ------------ |
|  📝  | **note**    | メッセージへの補足説明 | 仕様コメント、注意事項  |
|  ═   | **divider** | 区切り線        | フェーズ分け、処理区切り |
...
' ===== note: 補足説明（メッセージに注釈を付ける） =====
お客様 -> 受付システム: 予約申し込み
note right: （note）24時間受付可能

' ===== divider: 区切り線（処理フェーズを分ける） =====
==（区切り線）確認フェーズ ==
...
> **💡 補足説明にはnote、処理フェーズの区切りにはdividerを使用**

##### 応用タイプ（必要に応じて）

| S2表示 | タイプ | 説明 | 用途例 |
|:------:|--------|------|--------|
| ⋯⋯ | **delay** | 時間経過の表現 | 待機時間、タイムアウト |
| ◇ | **create** | オブジェクト生成 | インスタンス化、セッション開始 |
| × | **destroy** | オブジェクト破棄 | 終了処理、リソース解放 |
...
' ===== create: オブジェクト生成（途中で登場させる） =====
ユーザー -> アプリ: ログイン
create database セッションDB
アプリ -> セッションDB: セッション作成
...
' ===== destroy: オブジェクト破棄（終了を明示） =====
destroy セッションDB
アプリ --> ユーザー: 再ログインが必要です
@enduml
```

> **詳細**: [[#10. ノート入力UI|→ §10 ノート]] | [[#6.9 区切り要素（区切り線/スペース/遅延）|→ §6.9 区切り要素]] | [[#6.10 オブジェクトライフサイクル（create/destroy）|→ §6.10 create/destroy]]
...
### 6.4 表示方式: 方式I（D-47〜D-52, D-101）

| 要素 | 表現方法 | 特徴 |
|------|---------|------|
| **group** | ラッパー（色付きボーダー）+（☰ D&Dハンドル + [✏] + [🗑]） | 視覚的に囲まれた感、ネスト明確 |
| **activate** | 外部左縦線（縦位置揃え）+ **オブジェクトラベル縦書き** | PlantUML図と同じ表現、一直線 |
| **メッセージ** | カードUI（☰ D&Dハンドル + [✏] + [🗑]） | 編集・削除ボタン付き |
| **フラグメント** | ラッパー（条件ラベル付き）+（☰ D&Dハンドル + [✏] + [🗑]）+ **内部要素はカードUI** | alt/opt/loop等を視覚的に囲む。内部のメッセージ・ノートは個別カードUIで操作可能 |
| **ノート** | カードUI（☰ D&Dハンドル + 📝アイコン + [✏] + [🗑]） | メッセージカードの下に表示。編集・削除・並べ替えがメッセージと同等の操作性 |
...
> **参照**: [[#11.11 S2表示におけるgroup/アクティブバー表現（方式I採用）|→ §11.11 方式I詳細]]
...
├────────────────────────────────────────────────────────────────────────────────────┤
│                                                                                    │
│ ☰ 1. Alice → Bob: hello                                              [✏]     [🗑] │  ← 同期(→)
│ ☰    📝 「受信確認」                                                  [✏]     [🗑] │
│                                                                                    │
│ ┌─ ☰ 🔀 alt ────────────────────────────────────────────────────────[✏]──────[🗑]┐ │  ← フラグメント編集・削除
│ │  「認証成功」                                                        [✏]       │ │  ← 条件テキスト編集
│ │ ☰ 2. Alice → Server: データ取得                                    [✏]  [🗑] │ │  ← 同期(→)
│ │ ☰ 3. Server <-- Alice: データ返却                                  [✏]  [🗑] │ │  ← 戻り(<--)
│ ├─ else ──────────────────────────────────────────────────────────────────────────┤ │
│ │  「認証失敗」                                                        [✏]       │ │  ← 条件テキスト編集
│ │ ☰ 4. Server <-- Alice: エラー返却                                  [✏]  [🗑] │ │  ← 戻り(<--)
│ └─────────────────────────────────────────────────────────────────────────────────┘ │
│                                                                                    │
│ ☰ 5. Bob <-- Alice: done                                             [✏]     [🗑] │  ← 戻り(<--)
└────────────────────────────────────────────────────────────────────────────────────┘
...
> **UI統一原則（TD-037）**: 全S2要素（メッセージ・ノート・フラグメント・group・区切り要素・create/destroy）は `☰` D&Dハンドル + `[✏]` 編集 + `[🗑]` 削除のアイコンボタンで統一。`[編集 →]` のようなテキストボタンは使用しない。
...
| 操作                  | 動作                               | 展開方式  | 詳細セクション |
| ------------------- | -------------------------------- | :---: | :-----: |
| **[+ メッセージ]** クリック  | メッセージ 追加U                        | インライン |   §8    |
| **[+ 構造要素 ▼]** クリック | ドロップダウンメニューが展開                   | メニュー  |    -    |
| └ **フラグメント**        | フラグメント 追加UI                      | インライン |   §9    |
| └ **ノート**           | ノート 追加UI                         | インライン |   §10   |
| └ **─────────**     |                                  |       |         |
| └ **区切り線**          | 区切り線 追加UI                        | インライン |  §6.9   |
| └ **スペース**          | スペース 追加UI                        | インライン |  §6.9   |
| └ **遅延**            | 遅延 追加UI                          | インライン |  §6.9   |
| └ **─────────**     |                                  |       |         |
| └ **group**         | Group/Activate作成モーダル（groupタブ）    | モーダル  |   §11   |
| └ **アクティブバー**       | Group/Activate作成モーダル（activateタブ） | モーダル  |   §11   |
| └ **create**        | create 追加UI                      | インライン |  §6.10  |
| └ **destroy**       | destroy 追加UI                     | インライン |  §6.10  |
...
│ create           (§6.10) │ ← インライン展開
│ destroy          (§6.10) │ ← インライン展開
└───────────────────────┘
```

### 6.8 D&D並べ替え仕様（D-64〜D-70, TD-037）
...
### 6.10 オブジェクトライフサイクル（create/destroy）
...
| 操作                 | 動作             | 展開方式  |     |
| ------------------ | -------------- | :---: | :-: |
| **[+ メッセージ]** クリック | メッセージ追加フォームが展開 | インライン |     |
...
##### 6.11.4.1 return と自己参照の相互排他性

`return`と`↺ 自己`は**意味的に相互排他**であり、同時に選択することはできない。

| 特性 | return（← return） | 自己参照（↺ 自己） |
|------|-------------------|-------------------|
| **意味** | 直前の**呼び出し元**へ戻る | **自分自身**へメッセージを送る |
| **送信先決定** | 自動（呼び出しスタックから） | 明示（対象オブジェクト=送信元=送信先） |
| **用途** | activate区間の終了応答 | 内部処理、再帰呼び出し |
| **PlantUML構文** | `return result` | `User -> User: think` |

**「自分自身へのreturn」は成立しない理由**:
- `return`は「呼び出し元へ戻る」という意味であり、自己参照とは概念が異なる
- 自己参照メッセージ後に自分へ戻りたい場合は、通常の`<-- 戻り`（明示的戻りメッセージ）を使用する
- activate区間内での自己参照後の終了は、`return`で**直前の呼び出し元**へ戻る

**UIでの実装**:
- `↺ 自己`選択時、`← return`ボタンは非活性（disabled）
- `← return`選択時、`↺ 自己`ボタンは非活性（disabled）
- ツールチップ: 「returnと自己参照は同時に使用できません」
...
#### 6.12.2 入力方式: コンテナファースト + D&D（D-9, D-10, D-11）
...
#### 6.12.3 タイプ別UI要件（D-15, D-16, D-17）
...
##### 6.12.6.3 opt/loop/break（単純条件型）入力項目
...
##### 6.13.1.4 hnote（六角形ノート）

六角形の形状を持つノート。重要な情報や強調したい内容に使用。`over`位置のみ対応。
...
Alice -> Bob: request
hnote over Bob: ⚠ 重要な処理
hnote over Alice, Bob: この区間は\n特に注意が必要
Bob -> Alice: response
@enduml
```
...
##### 6.13.1.5 rnote（角丸四角形ノート）

角が丸い四角形のノート。ソフトな印象を与えたい補足説明に使用。`over`位置のみ対応。
...
背景色を指定してノートを強調表示。note/hnote/rnote すべてで使用可能。
...
hnote over Alice #yellow: ⚠ 注意事項
rnote over Bob #lightblue: 完了
...
#### 6.14.4 範囲選択方式（トグル+自動補完）（D-24, D-31）
...
**ネストアクティブバー**（Server内でDB呼び出し）:
- アクティブバー1: Server（M2-M6）
- アクティブバー2: DB（M3-M4、Server内でネスト）
...
**逐次アクティブバー**（複数オブジェクトが順番にアクティブ）:
- アクティブバー1: Server（M1-M7、全体を包含）
- アクティブバー2: Cache（M2-M3、Server内で完結）
- アクティブバー3: DB（M4-M5、Cache完了後に開始）
...
**多重アクティブバー**（同一オブジェクトの自己呼び出し）:
- アクティブバー1: Server（M1-M4、外側・メイン処理）
- アクティブバー2: Server（M2-M3、内側・validate）
...
#### 6.14.11 S2表示におけるgroup/アクティブバー表現（方式I採用）
...
┃                                                         ┃
┃ ║  ┌──────────────────────────────────────────────┐    ┃
┃ ║  │ ☰ 1.→ Client → Server: login       [✏][🗑] │    ┃ ← 縦線がラッパー内
┃ S  ┌──────────────────────────────────────────────┐    ┃
┃ e  │ ☰ 2.← Server → Client: token       [✏][🗑] │    ┃
┃ r  └──────────────────────────────────────────────┘    ┃
┃ v                                                       ┃
┃ e  ┏━━━ ☰ group: 内部処理 ━━━━━━━━━━━━━━━━━━━[✏][🗑]┓  ┃ ← ネストラッパー
┃ r  ┃                                               ┃  ┃
┃ ║  ┃ ║  ┌────────────────────────────────────────┐┃  ┃
┃ ║  ┃ D  │ ☰ 3.→ Server → DB: validate  [✏][🗑] │┃  ┃ ← 2本縦線がラッパー境界を貫通
┃ ║  ┃ B  └────────────────────────────────────────┘┃  ┃
┃ ║  ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛  ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```
...
│ │ r ║│ ┃ ☰ 3. Server → DB: query             [✏][🗑] ┃ │
│ │ v ║│ ┃ ☰ 4. Server <-- DB: result          [✏][🗑] ┃ │
│ │ e ║│ ┃ ☰ 5. Alice <-- Server: token        [✏][🗑] ┃ │
│ │ r  │ ┃                                                  ┃ │
│ └────┘ ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ │
│ ┌────┐                                                        │
│ │    │ ☰ 6. Alice <-- Bob: done                  [✏][🗑] │
...
│ │ r    │ ┃    ┃ ☰ 5. DB <-- Cache: hit                            [✏][🗑] ┃   ┃ │
│ │      │ ┃    ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛   ┃ │
│ │ D ║  │ ┃ ☰ 6. Server <-- DB: result                                  [✏]  [🗑] ┃ │
│ │ B ║  │ ┃ ☰ 7. Alice <-- Server: token                                [✏]  [🗑] ┃ │
│ │      │ ┃                                                                         ┃ │
│ └──────┘ ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛ │
│ ┌──────┐                                                                               │
│ │      │ ☰ 8. Bob → Alice: done                                          [✏]    [🗑] │
...
│                                    └─ [保存][キャンセル]                   　  │
│                                                                              │
│  [区切り要素カード] ───[✏]クリック───→ 区切り要素編集フォーム（§6.9.6）  　　　　　  │
│                                        ├─ ラベル編集（区切り線/遅延）      　   │
│                                        ├─ 高さ変更（スペース）             　　  │
│                                        └─ [保存][キャンセル]                　 │
│                                                                              │
...
| **group** | モーダル（§11.5） | モーダル（§11.6） | ダイアログ（§12.8） | §11 |
| **アクティブバー** | モーダル（§11.5） | モーダル（§11.6） | ダイアログ（§12.8） | §11 |
| **区切り線** | インライン（§6.9.3） | インライン（§6.9.6） | ダイアログ（§12.8） | §6.9 |
| **スペース** | インライン（§6.9.3） | インライン（§6.9.6） | ダイアログ（§12.8） | §6.9 |
| **遅延** | インライン（§6.9.3） | インライン（§6.9.6） | ダイアログ（§12.8） | §6.9 |
| **create** | インライン（§6.10.2） | インライン（§6.10.4） | ダイアログ（§12.8） | §6.10 |
| **destroy** | インライン（§6.10.3） | インライン（§6.10.4） | ダイアログ（§12.8） | §6.10 |

> **凡例**:
> - **インライン**: S1/S2セクション内にフォームを展開（§12.6参照）
> - **モーダル**: 全画面オーバーレイでLayer 2を開く（§12.6参照）
> - **ダイアログ**: 確認ダイアログを表示（§12.8参照、共通削除フロー）

---

## 9. AIコード適用 内部仕様（TD-028）
[[#目次|↑ 目次へ戻る]]
...
#### 9.6.1 複数コードブロック検出時のUI仕様（E-06対応）

AI回答に複数の`@startuml〜@enduml`ブロックが含まれる場合の警告表示仕様。

**検出タイミング**: `extractPlantUMLCode()`でmatches.length > 1の場合

**警告ダイアログUI**:

```
┌─────────────────────────────────────────────────────────────────────┐
│ ⚠️ 複数のPlantUMLコードブロックが検出されました                      │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│ AI回答に ${matches.length} 個のコードブロックが含まれています。     │
│ 最も長いコードブロック（${largest.lines}行）を適用します。          │
│                                                                     │
│ ┌─────────────────────────────────────────────────────────────────┐ │
│ │ 適用対象プレビュー（先頭5行）:                                   │ │
│ │ @startuml                                                       │ │
...
│ └─────────────────────────────────────────────────────────────────┘ │
│                                                                     │
│ ※ 他のコードブロック（${matches.length - 1}個）は無視されます      │
│                                                                     │
│                              [適用して続行]  [キャンセル]           │
└─────────────────────────────────────────────────────────────────────┘
```
...
| ボタン | 動作 |
|--------|------|
| **[適用して続行]** | 最大ブロックをCodeパネルに適用、§13.4処理続行 |
| **[キャンセル]** | 適用中止、AIチャットに戻る（ユーザーが手動選択可能） |

**代替案（MVP後検討）**: 複数ブロック選択UIの実装（ラジオボタンで選択）
...
### 10.3 D-47〜D-58, D-64〜D-70, D-101〜D-109: 表示・メッセージ・D&D・アクセシビリティ・パフォーマンス設計
...
| TD-037 | 全S2要素D&D可能（楽観的検証方式） | ☰ D&Dハンドル + [✏] + [🗑] 統一UI | §6.4, §6.7 |
| TD-038 | コンテキスト位置追加 | 選択要素直後に展開 | §5.4, §8.1, §9.2, §10.3 |
...
│ S2: 要素一覧                             │
├──────────────────────────────────────────┤
│ ☰ 1. User → Server: login        [✏][🗑] │  ← 矢印タイプ(→)+ラベル(:login)
├──────────────────────────────────────────┤
│ ⋮⋮ 新規メッセージ追加                     │  ← インライン展開されたフォーム
│ ├ 送信元: [User ▼]                       │
│ ├ 矢印:   [→|-->|<--|↺]                  │  ← 矢印タイプ選択
│ ├ 送信先: [Server ▼]                     │
│ ├ メッセージ: [________]                 │  ← メッセージラベル入力
│ └ [追加] [キャンセル]                    │
├──────────────────────────────────────────┤
│ ☰ 2. Server --> DB: query        [✏][🗑] │  ← 非同期メッセージの例
└──────────────────────────────────────────┘
```
...
## 13. APIファースト・マイクロモジュール設計詳細（TD-022）
[[#目次|↑ 目次へ戻る]]
...
| S2カード（メッセージ） | 「{送信元}から{送信先}へ{矢印タイプ}、{ラベル}」 | `aria-label` |
...
| 2026-01-03 | v3.27 | **残存課題解消（3件）**。**§12.9マトリクス改善**: group/アクティブバー/区切り要素/create/destroyの削除UIセクション参照を追加（§12.8共通削除フロー）、凡例追加（インライン/モーダル/ダイアログの説明）。**§13.6.1複数コードブロック検出時のUI仕様追加（E-06対応）**: 警告ダイアログUI（ASCII図）、適用対象プレビュー、ボタン動作（適用して続行/キャンセル）、MVP後の代替案（複数選択UI）を追加。**§8.4.1 returnと自己参照の相互排他性追加**: 意味的相互排他の根拠（呼び出し元vs自分自身）、特性比較表、「自分自身へのreturn」が成立しない理由、UIでの実装（相互disabled化、ツールチップ）を追加 |
| 2026-01-03 | v3.28 | **S1ボタン配置整合性修正**。**§5.3 UI設計図**: S1末尾の`[+ オブジェクトを追加]`ボタンを削除（ヘッダーの`[+]`のみに統一）。**§5.4.1 ボタンクリック時の動作**: 操作名を「ヘッダーの [+]」に変更、S2と同様の「フッターにはボタンを配置しない（ヘッダーのみ）」原則を明記。**§5.4 インライン展開図（パターン1/パターン2）**: 全ASCII図を「ヘッダーボタン起点」形式に更新（ヘッダー`▼ S1: オブジェクト [+]`を追加、末尾ボタンを削除）。S1/S2両方でヘッダーボタン方式が統一完了 |
| 2026-01-03 | v3.29 | **S1幅統一方針の整合性修正（v3.12漏れ対応）**。**§5.4.3タイトル修正**: 「フォームUI（600px幅）」→「フォームUI」（v3.12で他セクションから削除済みの「600px幅」がS1セクションで漏れていた）。**§5.3「幅に応じたレイアウト調整（機能は同一）」追加**: §6.5と同様の表を追加（コンパクト幅/標準幅で機能は同一、UIのみ変化）。設計方針「幅が狭い場合はUIをコンパクト化するが、機能は制限しない」を明記。S1/S2で幅に関する仕様が完全統一 |
| 2026-01-03 | v3.30 | **§11 コンパクト幅表示例の統一方針違反を修正**。**§11「表示例1: コンパクト幅」ASCII図修正**: 全カードに☰ D&Dハンドルと[✏]編集ボタンを追加（修正前は[🗑]削除ボタンのみ表示されており、「幅が狭くても機能は制限しない」という統一方針に違反していた）。標準幅・コンパクト幅ともに全機能ボタン（☰/✏/🗑）が表示されることを明示 |
| 2026-01-03 | v3.31 | **ASCII図アイコン統一（8箇所修正）**。統一方針「全要素に☰ D&D + [✏]編集 + [🗑]削除」を適用。**§4画面構成概要**: S1表示`[↑↓✎🗑]`→`☰ [✏][🗑]`、S2表示`≡`→`☰`+`✎`→`✏`に修正。**§6.8.5区切り要素S2表示**: 全5行に☰追加。**§6.9.4 create/destroy S2表示**: 全5行に☰追加。**§9.10.2フラグメント編集モーダル**: 内包メッセージに☰と[✏]追加（[🗑]のみ→☰[✏][🗑]）。**§12.4 S1オブジェクト追加フロー**: オブジェクトカードに☰[✏]追加、フッターボタンをヘッダー[+]方式に修正。**§13用語集インライン展開例**: `≡`→`☰`、[✏][🗑]追加 |
| 2026-01-03 | v3.32 | **TD-037「全要素D&D可能」に基づくASCII図修正（8箇所追加）**。**§4画面概要**: groupフラグメントヘッダーに☰追加。**§6.3表示方式表**: ノート定義に☰D&Dハンドル追加（「並べ替え」操作も明記）。**§6.4.4 S2表示例**: ノートカード📝に☰追加、altフラグメントヘッダー🔀に☰追加。**§8.9.2 UI設計**: 編集前メッセージカードに☰追加。**§9.10.2 UI設計**: 編集前altフラグメントカード⬡に☰追加。**§10.6.2 UI設計**: 編集前ノートカード□に☰追加。**§11アクティブバー図**: 入れ子groupフラグメントに☰追加 |
| 2026-01-03 | v3.33 | **TD-037統一設計方針の文書全体整合性修正**。**§6.3表**: group/フラグメント定義に`☰ D&Dハンドル + [✏] + [🗑]`追加。**§11.11方式I表**: group/メッセージ定義に`☰ D&Dハンドル + [✏] + [🗑]`追加。**§6.4 UI統一原則**: TD-037参照追加、全S2要素（group・区切り要素・create/destroy含む）を列挙、`☰`ハンドル明記。**ヘッダー対応TD範囲**: TD-017〜TD-035→TD-017〜TD-038に更新。**§15技術決定表**: TD-037（全S2要素D&D可能）・TD-038（コンテキスト位置追加）追加。**§14.3 D-53**: D&Dハンドル`⋮⋮`→`☰`に修正。**§8.5表**: D&Dハンドル`⋮⋮`→`☰`に修正 |
| 2026-01-04 | v3.34 | **§6.2 S2要素タイプ新設＋セクション番号振り直し**。S1「§5.2オブジェクトタイプ」と同等の入門性をS2に確保。**§6.2 S2要素タイプ追加**: メッセージ（矢印タイプ4種+PlantUML例）、構造要素（フラグメント7種+PlantUML例）、その他要素（note/divider/delay/create/destroy）、§8/§9/§10への参照リンク。**セクション番号振り直し**: 6.3統合ビュー→6.4表示方式→6.5 UI統合版→6.6 UI説明→6.7操作マッピング→6.8 D&D仕様→6.9区切り要素→6.10 create/destroy。**目次更新**: §6.2追加、番号更新。**内部参照更新**: §6.8→§6.9、§6.9→§6.10（本文+マトリクス表） |
| 2026-01-04 | v3.35 | **§6.2 S2要素タイプ表に「S2表示」アイコン列追加＋基本/応用分類**。§5.2オブジェクトタイプと同様の「基本タイプ（これだけ覚えればOK）」「応用タイプ（必要に応じて）」構成に統一。**メッセージ**: 基本2種（→同期、<--戻り）＋応用2種（-->非同期、↺自己参照）。**フラグメント**: 基本3種（🔀 alt/opt/loop）＋応用4種（🔀 break/par/critical/group）。**その他要素**: 基本2種（📝note、═══divider）＋応用3種（⋯⋯delay、◇create、×destroy）。全アイコンは§6.5/§6.9/§6.10のS2表示例と整合 |
| 2026-01-04 | v3.36 | **§6.2 PlantUMLサンプル追加（非エンジニア向け）**。日本語コメント付きの実務シナリオで各要素の使い方を視覚化。**基本フラグメント例**: 注文システムで`alt`（在庫あり/なし分岐）、`opt`（ポイント利用）、`loop`（商品ごと繰り返し）を解説。**応用フラグメント例**: レジ決済で`break`（エラー中断）、`par`（並列処理）、`critical`（排他制御）、`group`（締め処理まとめ）を解説。**基本その他要素例**: 予約受付で`note`（補足説明）、`==divider==`（フェーズ区切り）を解説。**応用その他要素例**: セッション管理で`create`（オブジェクト生成）、`...delay...`（タイムアウト）、`destroy`（オブジェクト破棄）を解説 |
| 2026-01-04 | v3.37 | **§10.1 hnote/rnoteセクション新設**。§10.1.0ノートタイプ一覧で定義済みのhnote/rnoteに対応する個別セクションを追加。**§10.1.4 hnote（六角形ノート）**: 重要情報・強調用、`over`位置のみ対応、単一/複数オブジェクト跨ぎのPlantUML例。**§10.1.5 rnote（角丸四角形ノート）**: ソフトな印象の補足説明用、`over`位置のみ対応、PlantUML例。**§10.1.6 背景色付きノート**: 旧10.1.4を繰り下げ、note/hnote/rnote全対応のPlantUML例を拡充（#yellow、#lightblue追加） |
| 2026-01-04 | v3.38 | **§11.10 複数アクティブバー適用例追加**。「複数タブ適用例」と同様に、アクティブバーの複合パターンをPlantUML例で解説。**ネストアクティブバー**: Server内でDB呼び出し（内側にネスト）。**並列アクティブバー**: 複数オブジェクト（Server/Cache）が同時にアクティブ。**多重アクティブバー**: 同一オブジェクトの再帰呼び出し（自己参照で内側にネスト）。3パターンで複雑なアクティブバー構成を網羅 |
| 2026-01-04 | v3.39 | **§11.10「並列→逐次アクティブバー」修正**。v3.38の「並列アクティブバー」例で、Server→DB矢印がCacheのアクティブバーを横切る問題を修正。**修正内容**: Cacheの応答完了後にDBを呼び出す逐次パターンに変更。DBにもactivate/deactivate追加。**命名変更**: 「並列アクティブバー」→「逐次アクティブバー」（複数オブジェクトが順番にアクティブ）。矢印がライフラインを横切らない正しいシーケンス図の例に修正 |
| 2026-01-04 | v3.40 | **§11.10「多重アクティブバー」修正**。自己呼び出し（再帰）のactivate不整合を修正。**修正前**: validateはactivateあり、sanitizeはactivateなし（不整合）。**修正後**: 3段ネスト構造に統一（最外側=process、中間=validate、最内側=sanitize）。各自己呼び出しにactivate/deactivateペアを追加し、コールスタックの深さを正確に視覚化 |
| 2026-01-04 | v3.41 | **§11.10「多重アクティブバー」簡素化**。3階層→2階層に変更。**理由**: 2階層が実務で最も一般的なパターン（自己呼び出し1段）。3階層は稀であり、例として過剰。**修正後**: 外側=process、内側=validate の2段構成。説明も「再帰呼び出し」→「自己呼び出し」に変更（再帰は深さを含意するため） |
| 2026-01-04 | v3.42 | **§11.10「多重アクティブバー」戻り矢印追加**。`deactivate Server`→`return validated`に変更。**理由**: deactivateのみでは自己呼び出しの始点と内側バーの接続が視覚的に不明確。`return`キーワードにより内側バーの終了と戻り値を明示し、矢印の流れが自然に。メッセージ番号もM1-M4（外側）、M2-M3（内側）に更新 |
| 2026-01-04 | v3.43 | **§6.4タイトル・参照修正**。「方式H」→「方式I」に修正。**背景**: 本仕様書で採用した表示方式は方式I（外部左カラム方式）であり、方式Hは比較検討で却下された案。**修正箇所**: §6.4タイトル、参照リンク（外部ドキュメント参照→§11.11内部参照に変更）。D-101を対応TDに追加 |
| 2026-01-04 | v3.44 | **§6.2 応用タイプに`return`追加**。PlantUMLの`return`キーワードを応用タイプ表に追加（S2表示: `2.⏎`）。**理由**: `return`は`<--`と異なり、戻りメッセージ生成と同時にアクティブバーを自動終了する。§11.10「多重アクティブバー」例で使用済み。**追加修正**: PlantUML例をアクティブバー付きに更新し、`return`の使用例を追加。§11.10への参照リンクも追加 |
