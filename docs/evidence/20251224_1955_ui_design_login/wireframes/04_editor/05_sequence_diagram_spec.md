...
| **AIチャット** | ドラッグで範囲選択 | ❌ | ❌ | AI回答から適用コードを選択 |
...
**重要**: AIには常に**Codeパネルの選択情報（行番号付き）+ 全体コード**が送信される。

| 選択元 | 同期動作 | AIへの送信 |
|--------|---------|-----------|
| GUIで選択 | Code行がハイライト | **Code行番号 + 全体コード** |
| Codeで選択 | GUIカードがハイライト | **Code行番号 + 全体コード** |
...
| 状態 | 展開位置 | 追加後の位置 |
|------|---------|-------------|
| **参加者未選択** | S1末尾（最終参加者の下） | 定義順の最後（図では右端） |
| **参加者選択中** | 選択参加者の**直後** | 選択参加者の次の位置 |

> **設計思想**: 参加者の定義順は図上の左右配置に対応。選択参加者の直後に追加することで、関連する参加者をグループ化しやすくなる。
...
|------|-----------------|-----------------|
| **要素未選択** | 範囲未選択 | S2末尾（最終カードの下） |
| **要素選択中** | 選択要素を**デフォルト範囲開始点**として選択 | 選択範囲の位置（ラップ） |

> **設計思想**: モーダル呼び出し前の選択状態をデフォルト範囲選択に反映することで、コンテキストに応じた操作が可能。group/アクティブバーは既存要素をラップするため、最終的な挿入位置は選択範囲で決まる。
...
| トリガー | 遷移先 | 展開方式 | 挿入位置（TD-038） | 入力項目 | 完了後 |
|---------|--------|:--------:|:------------------:|---------|--------|
| **S1: [+ 参加者を追加]** | 参加者追加フォーム | インライン | 選択参加者の直後 / 末尾 | タイプ、名前、表示名、エイリアス | S1リスト更新 |
| **S2: [+ メッセージ]** | メッセージ追加フォーム | インライン | 選択要素の直後 / 末尾 | 送信元、矢印タイプ、送信先、メッセージ、hidden | S2リスト更新 |
| **S2: [+ 構造要素] → フラグメント** | フラグメント追加フォーム | インライン | 選択要素の直後 / 末尾 | タイプ、条件ラベル（動的）、内包メッセージ | S2リスト更新 |
| **S2: [+ 構造要素] → ノート** | ノート追加フォーム | インライン | 選択要素の直後 / 末尾 | 位置、対象、内容、背景色 | S2リスト更新 |
| **S2: [+ 構造要素] → group** | Group/Activateモーダル | **モーダル** | 選択要素の直後 / 末尾 | ラベル、対象範囲（既存メッセージから選択） | S2リスト更新 |
| **S2: [+ 構造要素] → アクティブバー** | Group/Activateモーダル | **モーダル** | 選択要素の直後 / 末尾 | 対象参加者、対象範囲（既存メッセージから選択） | S2リスト更新 |

> **TD-038（コンテキスト位置追加）**:
> - **インライン**: 要素選択中はフォームを選択要素の直後に展開、未選択時は末尾に展開
> - **モーダル**: 要素選択中はモーダル完了後に選択要素の直後に挿入、未選択時は末尾に挿入（モーダル内のデフォルト範囲選択にも選択状態を反映）
...
| 2026-01-01 | v3.16 | **TD-038: コンテキスト位置追加（Context-aware Position Addition）**。全要素追加UIに展開位置ルールを追加。§5.5 参加者追加フォーム、§8.1 メッセージ追加フォーム、§9.2 フラグメント追加、§10.3 ノート追加、§11.1 Group/アクティブバーにTD-038展開位置ルール追加。§12.5 入力画面遷移マトリクスに「挿入位置（TD-038）」列追加、全6入力画面の挿入位置を統一表現。インライン展開: 選択要素の直後/末尾。モーダル: 選択要素をデフォルト範囲開始点、挿入位置は選択範囲。D-71/D-72/D-73追加。`technical_decisions.md`にTD-038を追加 |
