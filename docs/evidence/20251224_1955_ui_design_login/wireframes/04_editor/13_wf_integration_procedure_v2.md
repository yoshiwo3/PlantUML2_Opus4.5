...
> **評価履歴**: v1.0(82点) → v1.1(91点) → v1.2(95点) → v1.3(97点) → v1.4(97点/最終版) → **v2.5(94点/§0追加)**
>
> **ドキュメント構成**:
> | パート | セクション | 内容 |
> |:------:|-----------|------|
> | **Part 0: ルール** | §0 | **WF作成ルール参照【必読】** |
...
> **⚠️ 重要**: 作業開始前に**§0「WF作成ルール参照」を必ず読むこと**。
> 00_Session_Start.mdのルール適用範囲と特化ルールを確認してから作業開始。
...
│                                                                     │
│  Step 0: ルール確認（§0必読）                                       │
│    □ 00_Session_Start.md読了、§0.3チェックリスト確認               │
│                                                                     │
│  Step 1: バックアップ作成                                           │
...
**所要時間目安**: §0確認 10分、01_default統合 2時間、06_gui_only作成 1時間

---

## 0. ワイヤーフレーム作成ルール参照【必読】

> **参照元**: `docs/guides/ui_design/00_Session_Start.md` v1.11
> **適用範囲**: 本手順書の統合作業に適用するルールを明示
> **関係性**: 本セクションは00_Session_Start.mdのルールを**統合作業に特化して適用**したもの

### 0.1 適用ルール一覧

本手順書の統合作業は、一般的なワイヤーフレーム作成プロセスの**一部**として実行されます。以下のルールが適用されます。

| # | ルール | 00_Session_Start.md参照 | 本手順書での適用 |
|:-:|--------|------------------------|-----------------|
| R1 | **必読ファイル確認** | §0 必読ファイル | ✅ 適用（§0.3で確認）|
| R2 | **TD-015原則** | §3 Step 5 | ✅ 適用（低精度、手書き風、グレースケール）|
| R3 | **5パスレビュー** | §3 Step 7 | ✅ 適用（統合後に実施）|
| R4 | **90点以上合格** | §3 Step 7 | ✅ 適用（§6成功基準で検証）|
| R5 | **00_wireframe_index.md即時更新** | §3 Step 5, 8 | ✅ 適用（§4で詳細化）|
| R6 | **参照ドキュメント活用** | §4 参照ドキュメント | ✅ 適用（Knowledge_Base, Patterns参照）|
| R7 | **Evidence 3点セット** | §5 完了条件 | ✅ 適用 |
| R8 | **SERENA Memory保存** | §3 Step 8 | ✅ 適用 |
| R9 | **Git commit** | §3 Step 8 | ✅ 適用 |

### 0.2 本手順書に特化したルール

以下のルールは、統合作業の特性に合わせて**特化した手順**を適用します。
（00_Session_Start.mdの基本原則は維持しつつ、手順を統合作業向けに調整）

| # | ルール | 00_Session_Start.md | 本手順書での特化内容 | 特化理由 |
|:-:|--------|---------------------|----------------------|---------|
| S1 | **作業単位** | 1セッション=1画面 | **1セッション=統合作業全体**（01_default統合 + 06_gui_only作成）| 統合作業は複数ファイルを連続処理するため |
| S2 | **作成手順** | Step 1〜8 | **§3統合手順（Phase 1〜4）**を使用 | 統合には座標変換・要素選択等の専用手順が必要 |
| S3 | **検証チェックリスト** | 5パスレビュー | **§3.4, §6の専用チェックリスト**を使用 | 統合特有の検証項目（座標精度、要素重複等）が必要 |
| S4 | **ドキュメント更新** | Step 8の1〜7 | **§4の2段階更新（更新A/B）**を使用 | 統合は2段階で完了するため更新タイミングが異なる |

### 0.3 作業開始前の確認事項

**統合作業開始前に以下を確認すること**（00_Session_Start.md §0, §1準拠）:

```
□ 0-1: CLAUDE.mdを読んだ
□ 0-2: active_context.mdで現在の進捗を確認した
□ 0-3: 00_Session_Start.md（本ルールの参照元）を読んだ
□ 0-4: UI_Design_Constitution.md（憲法）を読んだ
□ 0-5: 本手順書（Part 0〜D）の構成を理解した
□ 0-6: ユーザーから統合作業の承認を得た
```

> **Note**: 必要に応じて以下の参照ドキュメントも確認（00_Session_Start.md §4）:
> - `UI_Design_Knowledge_Base.md`: 過去の知見（EX/SD/TD/IC/CS）
> - `UI_Design_Patterns.md`: UIパターン集（EP/SP）

### 0.4 完了条件（統合ルール適用）

**統合作業の完了条件**（00_Session_Start.md §5を統合作業に適用）:

> **重要**: 本チェックリストは§6.1「統合完了の必須条件（C1〜C8）」を**補完**するものです。
> §6.1と本チェックリストの**両方**をパスする必要があります。

```
統合作業完了チェックリスト:
───────────────────────────────────────────────────────
【品質基準】
□ C-A: TD-015原則を遵守した統合結果になっている
□ C-B: Visual Check (V1〜V5) 通過（§3.2.3, §3.3.2参照）
□ C-C: AI評価 90点以上達成（§6.2品質基準で判定）
□ C-D: §6.1の必須条件（C1〜C8）すべてパス ← 必須参照

【ドキュメント更新】
□ C-E: 00_wireframe_index.md更新（§4の10箇所）完了
□ C-F: active_context.md更新

【Evidence・記録】
□ C-G: Evidence更新（work_sheet.md, 00_raw_notes.md）
□ C-H: SERENA Memory保存
□ C-I: Git commit
───────────────────────────────────────────────────────
```
...
統合作業でも**イテレーションループ**を適用します（00_Session_Start.md §3 Step 5, 9準拠）:

> **ループ制限**: 各ループ最大**3回**まで。3回で90点未満の場合は問題点を明記してユーザーに相談。

```
┌─────────────────────────────────────────────────────────────────────┐
│  イテレーションループ1（AI自己評価後）【最大3回】                   │
├─────────────────────────────────────────────────────────────────────┤
│  統合実行 → Visual Check (V1〜V5) → 90点未満？                     │
│                                         ↓ Yes（ループ回数 < 3）     │
│                                   修正 → 再度Visual Check           │
│                                         ↓ 90点以上                  │
│                                   ユーザーに提示                    │
│                                                                     │
│  ※3回で90点未満 → 問題点リストを作成し、ユーザーに判断を仰ぐ      │
└─────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────┐
│  イテレーションループ2（ユーザー編集後）【最大3回】                 │
├─────────────────────────────────────────────────────────────────────┤
│  ユーザー編集 → AI評価 (§6チェックリスト) → 90点未満？            │
│                                               ↓ Yes（ループ回数 < 3）│
│                                         修正 → 再評価               │
│                                               ↓ 90点以上            │
│                                         成果物保存                   │
│                                                                     │
│  ※3回で90点未満 → 問題点リストを作成し、継続/中断を判断            │
└─────────────────────────────────────────────────────────────────────┘
```
...
| v2.5 | 80→94点 | §0「WF作成ルール参照」追加（00_Session_Start.md統合）、3回フィードバックループ |

*v2.4（最終版）- §4更新手順の3回フィードバックループを経て完成*
*v2.5 - §0「WF作成ルール参照」追加、00_Session_Start.mdとの統合（3回ループ: 80→90→94点）*
