...
| 3 | **03_ai_code_apply** | AIコード適用フロー | TD-028 §11.2, §11.4-11.7 | **P1** | ✅ 100点 |
...
**表現する状態**:
- PlantUML構文エラー発生
- PreviewにPlantUML **JARが生成したエラー画像**を表示（自動）
- Codeパネルにエラー行ハイライト
...
| エリア | 要素 | 備考 |
|--------|------|------|
| **Codeパネル** | エラー行ハイライト（黄色背景） | カスタムUI |
| **Codeパネル** | ガター赤●アイコン | 行番号横 |
| **Previewパネル** | JARが生成したエラー画像 | **自動表示（カスタムUI不要）** |
| **AIチャット** | **[再生成]ボタン（既存）** | 右上に配置済み（01_defaultから存在） |
| **AIチャット** | **エラー通知なし** | 会話履歴汚染防止 |

**注釈**:
- TD-028 §11.8 参照
- **「PreviewパネルはJARの出力を表示するのみ、カスタムUI配置不可」**
- **「再生成はAIパネル右上の既存ボタンを使用」**
