# エディタ画面構成 ベストプラクティス分析 v10.0

**作成日時**: 2025-12-28 18:00
**最終更新**: 2025-12-29（v10.0 - ブラッシュアップ版）
**対象**: #04 エディタ画面
...
### 2.1 基本レイアウト（TD-023: エディタ画面基本レイアウト）[UC 3-1〜3-6, 4-1, 4-2]
...
### 2.2 GUIパネル内部構造（TD-019 層1）[UC 3-1, 3-3] ⚡ **v2.1更新**
...
### 2.3 2層統合モーダル構造（TD-019 v2.0）[UC 3-1, 3-2] ⚡ **Session 13で更新**
...
### 2.4 2レベルGUI編集構造（TD-020 v2.0）[UC 3-3] ⚡ **Session 13で更新**
...
### 2.8 キーボードショートカット一覧 [UC 3-1〜3-6, 4-1, 4-2]
...
> **対象画面**: #04 エディタ画面
> **対応UC**: 8件（UC 3-1〜3-6, UC 4-1〜4-2）

| カテゴリ | UC | 内容 |
|---------|:--:|------|
| 図表操作 | UC 3-1 | 図表作成 |
| 図表操作 | UC 3-2 | テンプレート選択 |
| 図表操作 | UC 3-3 | コード編集 |
| 図表操作 | UC 3-4 | プレビュー |
| 図表操作 | UC 3-5 | 保存 |
| 図表操作 | UC 3-6 | エクスポート |
| AI機能 | UC 4-1 | AI Question-Start |
| AI機能 | UC 4-2 | AIチャット |

**対象外UC（24件）**:
- 認証（UC 1-1〜1-2）: ログイン画面で対応
- プロジェクト管理（UC 2-1〜2-4）: ホーム画面・モーダルで対応
- 図表操作残り（UC 3-7〜3-11）: v3機能、学習コンテンツ
- 管理機能（UC 5-1〜5-13）: 管理画面群で対応
...
```typescript
/**
 * GUIパネルまたはCodeパネルで選択された要素を表す
 * AIコード適用機能で対象コードを特定するために使用
 */
interface SelectedElement {
  /** 要素のテキスト内容（例: "Alice -> Bob: Hello"） */
  text: string;
  /** コード内の行番号（1始まり） */
  lineNumber: number;
}
...
/**
 * AI再生成時にエラー修正を依頼するためのリクエストデータ
 * 試行履歴を含むことで、AIが過去の失敗を考慮した修正を生成可能
 */
interface ErrorFixRequest {
  /** 元のユーザー依頼テキスト */
  originalRequest: string;
  /** PlantUMLレンダラーが返したエラーメッセージ */
  errorMessage: string;
  /** エラーが発生した行番号 */
  errorLineNumber: number;
  /** エラー行 ± 5行のコードコンテキスト */
  surroundingCode: string;
  /** 完全なPlantUMLコード（@startuml〜@enduml） */
  fullCode: string;
  /** 過去の試行履歴（最大5回） */
...
/**
 * PlantUMLレンダリング時のエラー情報
 * Previewパネルのエラーバナー表示に使用
 */
interface ErrorInfo {
  /** エラーメッセージ */
  message: string;
...
interface Constitution {
  ...
  /** 必須ルール（AIが必ず従うべき制約） */
  requirements?: string[];
  /** 禁止事項（AIが絶対に行ってはならない事項） */
  prohibitions?: string[];
  /** 推奨事項（可能であれば従うべきガイドライン） */
 ...
 * ファイルシステムまたはリモートから憲法定義を取得
 * @see §11.9 階層的憲法システム
 */
interface IConstitutionProvider {
  /**
   * 指定パスの憲法を読み込む
   * @param path - 憲法ファイルパス（例: "base.yaml", "llm/claude.yaml"）
   * @returns 憲法オブジェクト、見つからない場合はnull
   */
   ...
   * 指定憲法のバージョンを取得
   * @param path - 憲法ファイルパス
   * @returns バージョン文字列、見つからない場合はnull
   */
...
interface IConstitutionResolver {
  /**
   * 指定されたLLM・図表タイプに対応する憲法を解決
   * @param llm - LLMタイプ（例: 'claude', 'gpt4'）
   * @param diagramType - 図表タイプ（例: 'sequence', 'class'）
   * @returns マージ済み憲法オブジェクト
   */
...
/**
 * AIプロンプトを構築するビルダー
 * 憲法・コンテキストを組み合わせて最適なプロンプトを生成
 */
interface IPromptBuilder {
  /**
   * プロンプトを構築する
   * @param context - プロンプト構築に必要なコンテキスト情報
...
/**
 * プロンプト構築時のコンテキスト情報
 * AI呼び出しの種類に応じて必要なフィールドが異なる
 */
interface PromptContext {
  /** プロンプトタイプ: apply（適用）, error_fix（エラー修正）, generate（新規生成） */
...
```typescript
/**
 * Monaco Editorとの連携API
 * Codeパネルの編集・選択・装飾を制御
 * @see https://microsoft.github.io/monaco-editor/typedoc/interfaces/editor.IStandaloneCodeEditor.html
 */
interface IMonacoEditorAPI {
  /**
   * エディタの全テキストを取得
   * @returns PlantUMLコード全体
   */
  getValue(): string;

  /**
   * エディタのテキストを設定
   * @param value - 設定するPlantUMLコード
   */
  setValue(value: string): void;

  /**
   * 現在の選択範囲を取得
   * @returns 選択範囲オブジェクト、選択がない場合はnull
   */
  getSelection(): ISelection | null;

  /**
   * 選択範囲を設定
   * @param selection - 設定する選択範囲
   */
  setSelection(selection: ISelection): void;

  /**
   * デコレーション（ハイライト等）コレクションを作成
   * @param decorations - デコレーション定義の配列
   * @returns デコレーションコレクション（後で削除可能）
   */
  createDecorationsCollection(decorations: IModelDeltaDecoration[]): IEditorDecorationsCollection;

  /**
   * カーソル位置を取得
   * @returns カーソル位置、フォーカスがない場合はnull
   */
  getPosition(): IPosition | null;

  /**
   * カーソル位置を設定
   * @param position - 設定するカーソル位置
   */
...
interface IModelDecorationOptions {
  ...
  /** 適用するCSSクラス名（背景色等） */
  className?: string;
  /** グリフマージン（行番号左）のCSSクラス名 */
  glyphMarginClassName?: string;
  /** ホバー時に表示するMarkdownメッセージ */
...
> **本セクションの役割**: **決定事項（What）** のみを記載
> **決定経緯（Why/How）**: [付録A.2](#a2-session-13-phase-6-td-028-aiコード適用機能) 参照

> **型定義参照**: 本セクションで使用する型（`SelectedElement`, `ErrorFixRequest`, `IPromptBuilder`等）は **[§9 型定義一覧](#9-型定義一覧完全版)** を参照
...
│        │             │                                                      │
│        │  12│ ★黄色  │  ┌──────────────────────────────────────────────┐   │
│        │  ハイライト │  │ [PlantUML JAR生成のエラー画像]              │   │
│        │             │  │ ⚠️ 構文エラー（12行目）                     │   │
│        │             │  │ Expected 'as' keyword                       │   │
│        │             │  └──────────────────────────────────────────────┘   │
│        │             │                                                      │
├────────┴─────────────┴──────────────────────────────────────────────────────┤
│ AIチャットパネル（エラー通知なし）                        [再生成] ← 既存   │
└─────────────────────────────────────────────────────────────────────────────┘
```

> **重要**: PreviewパネルはPlantUML JARの出力を表示する領域であり、カスタムUIコントロール（ボタン等）は配置不可。再生成はAIチャットパネル右上の既存ボタンを使用。

#### 11.8.2 エラー表示仕様

| 項目 | 仕様 |
|------|------|
| **Previewパネル** | PlantUML JARが生成したエラー画像を自動表示 |
| **エラー画像内容** | ⚠️警告アイコン + エラー種別（行番号）+ エラーメッセージ |
| **再生成ボタン** | **AIチャットパネル右上の既存ボタンを使用** |
| **Codeパネル** | エラー行ハイライト（黄色背景）+ ガッター赤●アイコン |
...
| ID | シナリオ | 期待動作 | 実装方針 |
|:--:|---------|---------|---------|
| E-01 | ネットワークエラー（AI通信失敗） | トースト表示 + リトライボタン | try-catch + 再試行UI |
| E-02 | AIタイムアウト（30秒超過） | トースト表示 + キャンセル | AbortController使用 |
| E-03 | 空のコード選択 | 適用ボタン無効化 | disabled属性 |
| E-04 | 不完全なコード選択（@startumlのみ） | エラーメッセージ表示 | バリデーション |
| E-05 | AI応答にコードブロックなし | 再生成ボタン表示 | 正規表現チェック |
| E-06 | 複数の@startuml〜@endumlブロック | 最大ブロックを使用 | length比較 |
| E-07 | 構文エラー検出（レンダリング失敗） | エラー通知バナー + 再生成ボタン | §11.8参照 |
| E-08 | 再生成5回超過 | 警告表示 + ボタン無効化 | カウンター管理 |

### 11.11 AIコード適用 状態遷移

| 現状態 | イベント | 次状態 | アクション |
|--------|---------|--------|-----------|
| Idle | GUIパネル要素クリック | Selecting | pushElement(element) |
| Idle | Codeパネルトグル ON | CodeSelecting | enableCodeSelection() |
| Selecting | クリアボタン | Idle | clearSelection() |
| Selecting | AIチャット送信 | WaitingAI | sendToAI(prompt) |
| CodeSelecting | 範囲選択完了 | Selecting | setCodeRange(range) |
| CodeSelecting | トグル OFF | Idle | disableCodeSelection() |
| WaitingAI | AI応答受信 | ReviewingCode | displayAIResponse() |
| WaitingAI | タイムアウト | Error | showTimeoutError() |
| ReviewingCode | コード選択 + 適用ボタン | Applying | applyCode() |
| ReviewingCode | キャンセル | Idle | clearAll() |
| Applying | 適用成功 | Idle | updateEditor() + showSuccess() |
| Applying | 構文エラー | Error | showErrorBanner() |
| Error | 再生成ボタン | WaitingAI | regenerate() |
| Error | 閉じるボタン | Idle | clearError() |
...
| 日時 | バージョン | 内容 |
|------|:----------:|------|
| 2025-12-29 | **v10.0** | **ブラッシュアップ版**: §7.5スコープ定義追加、§2.xにUCタグ付与、§9全interfaceにJSDoc追加、§11.10エッジケース一覧追加、§11.11状態遷移表追加、§11/付録A役割宣言追加、型定義参照ノート追加 |
...
> **本セクションの役割**: **議論経緯（Why/How）** の詳細記録
> **決定事項（What）**: [§11 TD-028詳細設計](#11-td-028-aiコード適用機能-詳細設計) 参照
