...
| **ダッシュボード** | ログイン後に最初に表示される「ホーム画面」 | スマホのホーム画面 |
...
| **LLM（エルエルエム）** | 大規模言語モデル。ChatGPTのようなAI | 何でも答えてくれる専門家 |
...
<details>
<summary>📘 技術詳細を見る（エンジニア向け）</summary>
...
**この機能が解決する問題**:
アプリごとに新しいパスワードを作ると、覚えきれないし、使い回しによるセキュリティリスクも増えます。既に信頼しているGitHub/Googleの認証を「借りる」ことで、安全かつ便利にログインできます。
...
**💡 この機能が解決する問題**:
- 図表がたくさんあると、どれがどの案件のものか分からなくなる
- プロジェクトで分けることで、関連する図表をまとめて管理できる

##### 🎬 操作の流れ

```mermaid
flowchart LR
    A["📁 ダッシュボード"] --> B["➕ 新規プロジェクト"]
    B --> C["✏️ 名前を入力"]
    C --> D["✅ 作成完了"]
    D --> E["📂 新しいプロジェクトが開く"]
```
...
| # | あなたの操作 | 画面の様子 |
|:-:|------------|-----------|
| 1 | ダッシュボードで「新規プロジェクト」ボタンをクリック | ダイアログが表示される |
...
**💡 この機能が解決する問題**:
- 複数の案件を並行して進めているとき、スムーズに切り替えたい
- 前回の作業を中断したところから再開したい
...
**💡 便利な点**: 最後に選んだプロジェクトは記憶されます。次回ログイン時、自動的に前回のプロジェクトが開きます（別のパソコンからでも！）。
...
**⚠️ 重要な注意**: この操作は**取り消せません**。プロジェクト内の図表も**すべて削除**されます。
...
**💡 この機能が解決する問題**:
- PlantUMLの構文を覚えていなくても、ひな形を見ながら修正できる
- 「この種類の図を作りたい」がすぐ始められる
...
**💡 この機能が解決する問題**:
- PlantUMLはVS Code同様の高機能エディタで効率よく書ける
- Excalidrawは直感的なお絵かきツールで図形を配置できる
...
**💡 この機能が解決する問題**:
- 作成した図をWord、PowerPoint、Webページなどに貼り付けられる
- 印刷用に高品質なPDFを生成できる
...
**💡 この機能が解決する問題**:
- PlantUMLの書き方を調べるために外部サイトを検索する手間が省ける
- AIが質問を理解して、適切な回答を生成してくれる

**⚠️ 注意**: この機能はPhase 2で実装予定です。現在は利用できません。
...
**💡 この機能が解決する問題**:
PlantUMLの構文を覚えなくても図表が作れます。「ログイン機能のシーケンス図を作りたい」と入力すれば、AIが質問しながら必要な情報を引き出し、コードを生成します。
...
**💡 この機能が解決する問題**:
「この設計で大丈夫？」「何か不足している？」といった疑問に、AIがプロジェクトの状況を理解した上でアドバイスします。
...
**この機能が解決する問題**:
サービスを安全に運用するには、誰がどの権限でアクセスしているかを把握し、必要に応じて制御する必要があります。この機能で、ユーザー一覧を確認し、問題のあるアカウントを無効化したり、信頼できるユーザーに開発者権限を付与できます。
...
**この機能が解決する問題**:
AIモデルは日々進化し、新しいモデルが続々と登場します。この機能があれば、新しいモデルをコード変更なしで追加でき、コスト情報も正確に管理できます。接続テスト機能で、本番利用前に動作確認もできます。
...
**この機能が解決する問題**:
すべての機能に高性能AIを使うとコストがかさみます。一方、すべてに低コストAIを使うと品質が下がります。この機能で、機能ごとに最適なモデルを割り当て、コストと品質のバランスを取れます。
...
**この機能が解決する問題**:
AIの応答品質はプロンプト（指示文）に大きく依存します。この機能で、プロンプトをコード変更なしで編集・テストでき、AIの振る舞いを素早く改善できます。例えば「より詳しく説明してほしい」「日本語で回答してほしい」といった調整が可能です。
...
**この機能が解決する問題**:
AIの応答は「創造的すぎて不正確」になったり「正確だけど面白みがない」になったりします。パラメータを調整することで、用途に応じた最適なバランスを見つけられます。
...
**この機能が解決する問題**:
複雑なタスクは、1回のAI呼び出しでは対応できません。例えば「コードを分析→問題点を抽出→修正案を提示→修正案を検証」といった多段階処理が必要です。この機能で、そうした処理を定義・自動化できます。
...
**この機能が解決する問題**:
AIサービスは従量課金のため、使いすぎるとコストが膨らみます。この機能で使用量をリアルタイムに監視し、予算を超えそうな時にアラートを受け取れます。また、どの機能・ユーザーがコストを使っているかを分析し、最適化できます。
...
**この機能が解決する問題**:
AIサービスは障害が起きることがあります。メインのモデルが使えなくなっても、バックアップモデルに自動で切り替わることで、ユーザーへの影響を最小限に抑えられます。
...
**この機能が解決する問題**:
ヘルプドキュメントを検索する際、キーワード完全一致では見つからないことがあります。Embeddingモデルを使うと「意味的に近い」文書を検索できます。この機能で、検索精度を最適化できます。
...
**この機能が解決する問題**:
Embeddingもトークン単位で課金されます。F-ADM-07（LLM使用量監視）と合わせて、AI関連の総コストを把握できます。
...
**この機能が解決する問題**:
ユーザーが困った時に、適切なヘルプを見つけられることが重要です。この機能でヘルプドキュメントを登録すると、エディタ内ヘルプ機能（F-DGM-10）から検索できるようになります。
...
**💡 この機能が解決する問題**:
登録したヘルプドキュメントは、時間とともに古くなったり、不要になったりします。また、Embedding設定を変更した場合、既存コンテンツのベクトルを再生成する必要があります。この機能でこれらの管理作業を行えます。
...
**💡 この機能が解決する問題**:
アプリケーションの動作は多くの設定に依存します。新機能のリリース時にはフラグで段階的に公開したり、負荷対策として制限値を調整したりする必要があります。この機能で一元管理することで、安全に運用できます。
...
<details>
<summary>📘 業務フロー・DFD対比表を見る（エンジニア向け）</summary>
...
<details>
<summary>📘 機能依存関係図を見る（エンジニア向け）</summary>
...
<details>
<summary>📘 リスク分析を見る（エンジニア向け）</summary>
...
<details>
<summary>📘 クラス図への橋渡しを見る（エンジニア向け）</summary>

### 12.1 抽出エンティティ候補

| エンティティ | 説明 | 関連機能 |
|-------------|------|---------|
| **User** | システム利用者 | F-AUTH-01, F-AUTH-02, F-ADM-01 |
| **Project** | 図表をグループ化する単位 | F-PRJ-01〜04 |
| **Diagram** | PlantUML/Excalidraw図表 | F-DGM-01〜09 |
| **Template** | 図表作成用テンプレート | F-DGM-02 |
| **Session** | 認証セッション情報 | F-AUTH-01, F-AUTH-02 |
| **LLMModel** | 登録済みLLMモデル | F-ADM-02, F-ADM-03 |
...
**作成完了**: 2025-12-13 v2.0（非エンジニア向け改訂版）
