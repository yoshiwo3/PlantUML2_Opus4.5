...
### 3.6 概要図
- 保存 / エクスポート / 編集継続 の3分岐（手動保存のみ）
- PlantUML制限回避のためswitch内スイムレーン変更を削除
- **2025-12-06追記**: TD-006対応で自動保存を削除

### 3.6.1.1 PlantUML保存フロー（13ステップ）
| ステップ | 処理内容 | 担当 |
|:-------:|---------|------|
| 1 | 保存トリガー（手動のみ: Ctrl+S/保存ボタン） | エンドユーザー |
...
### project_brief.md大幅更新

| 追加セクション | 内容 |
|---------------|------|
| サービスアーキテクチャ | 内部5サービス、外部5システム、データフロー概要 |
| UX設計思想 | 5ペルソナ、5 UXポイント |
| 検証済み技術仕様 | Supabase SSR、Excalidraw、PlantUML構文 |
...
| 追加セクション | 内容 |
|---------------|------|
| 作業完了時の更新ルール | 必須3件、状況別4件、フローチャート、チェックリスト |

## 次のアクション

1. ~~**3.7 バージョン管理フロー**（UC 3-7, 3-8）~~ ✅ 完了
2. ~~**3.8 図表削除フロー**（UC 3-9）~~ ✅ 完了
3. **データフロー図（DFD）**
4. **機能一覧表**
