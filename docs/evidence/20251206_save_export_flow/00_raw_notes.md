...
## 自動保存機能削除（2025-12-06 追記）

### 背景
TD-006（Storage Only構成）決定により、30秒間隔の自動保存機能が実現困難と判明。
...
- 自動保存はv3（DB導入後）で検討
- 代替として「未保存警告（ページ離脱時）」を実装

### 修正内容（全13箇所）
- 業務フロー図: 8箇所（概要図、PlantUML/Excalidraw保存フロー、テーブル等）
- ユースケース図: 2箇所（note、UC 3-5テーブル）
- ~~開発ステップ詳細化計画: 1箇所（Day 4タスク）~~ ※2025-12-13廃盤
