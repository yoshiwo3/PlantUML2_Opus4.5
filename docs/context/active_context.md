...
**最終更新**: 2025-12-27（ワイヤーフレームフォルダ再構成、画面遷移図v1.2、学習コンテンツ画面追加）
...
| ⑨ 画面遷移図 | 🟡 | 98点 | 18ノード定義済み（v1.2 学習コンテンツ画面追加）、SVG未生成 |
| ⑩ ワイヤーフレーム | 🟡 | 100点 | 2/18（#1 ログイン✅、#2 ダッシュボード✅）|
| ⑪ コンポーネント図 | 🔴 | - | 要作成 |
| ⑫ 外部インターフェース一覧 | 🔴 | - | 要作成 |
| ⑬ 非機能要件一覧表 | 🔴 | - | 要作成 |
| ⑭ アクター権限マトリクス | 🔴 | - | 要作成 |
...
| 4 | エディタ画面 | editor | UC 3-1〜3-6, 4-1, 4-2 | 🔴 | ✅ | MVP |
...
- **遷移図ノード**: 18/18 完了（100%）✅
- **ワイヤーフレーム**: 2/18 完了（11%）🟡
- **MVP画面**: 16画面 / **Phase 2画面**: 2画面

**遷移図ファイル**: `docs/proposals/diagrams/09_screen_transition/99_screen_transition.puml`（v1.2）
...
| 優先度 | 図表 | ツール | 状況 | 備考 |
|:------:|------|--------|:----:|------|
| ~~0~~ | ~~画面一覧確定~~ | - | ✅ | 17画面確定（OAuth-only対応でsignup/reset削除）|
| **1** | **⑩ ワイヤーフレーム** | Excalidraw | 🟡 進行中 | 1/17（#1 ログイン✅、#2 ダッシュボードから継続）|
| ~~2~~ | ~~⑨ 画面遷移図~~ | PlantUML | 🟡 | 17ノード定義済み（v1.1）、SVG未生成 |
