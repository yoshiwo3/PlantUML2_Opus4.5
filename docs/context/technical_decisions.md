...
> **延期理由**: シーケンス図以外の図表タイプ（クラス図、ユースケース図等）のGUI設計が未完了。
> **MVP対応**: シーケンス図GUIのみ実装。他タイプはCodeパネル直接編集。
> **v1.1対応**: 他図表タイプのGUI順次対応（クラス図→ユースケース図→アクティビティ図）
...
> **延期理由**: 「適用」ボタンの適用ロジック（挿入/置換/マージ）が未定義。既存コードとの統合方法が複雑。
> **MVP対応**: コピーボタンのみ提供。ユーザーが手動でCodeパネルに貼り付け。
> **v1.1対応**: 差分プレビュー表示、適用位置選択（末尾追加/カーソル位置/全置換）、Undo連携。
