...
**インライン展開方式に対する選択状態ベースのコンテキスト位置追加**を採用。

| 方式 | 展開/挿入位置 | 適用対象 | TD-038 |
|------|--------------|---------|:------:|
| **インライン** | 選択要素の直後 / 未選択時は末尾 | メッセージ、参加者、フラグメント、ノート | ✅ 適用 |
| **モーダル** | モーダルで選択した範囲の位置（ラップ） | group、アクティブバー | ❌ 適用外 |

> **重要**: group/アクティブバーは「既存メッセージをラップする」という性質上、S2表示位置はモーダルで選択した範囲によって決まる。追加ボタンクリック時のカード選択状況は影響しない。

---

### 3. 動作仕様

#### 3.1 インライン展開方式（TD-038適用）
...
#### 3.2 モーダル方式（group/アクティブバー）- TD-038適用外
...
モーダルで選択した範囲:
│ 1. Alice → Bob: init        │
│ 2. Bob → Server: request    │  ← 選択開始
│ 3. Server → DB: query       │
│ 4. DB → Server: result      │  ← 選択終了
│ 5. Bob → Alice: done        │
...
| 項目 | 改善内容 |
|------|---------|
| **操作効率** | 中間位置への追加時にD&D不要（インライン展開のみ） |
...
| **D-72** | ~~モーダル: 選択要素をデフォルト範囲開始点として使用~~ → **削除**（モーダルは選択範囲に依存、追加ボタン時のカード選択は影響しない） |
...
| 2026-01-01 | ユーザー | **訂正**: group/アクティブバーは選択範囲に依存、追加ボタン時のカード選択は影響しない |
