...
**作成日**: 2025-12-08
**最終更新**: 2025-12-14
**バージョン**: 3.5（非エンジニア向け改訂版）
**対象範囲**: 全32ユースケース（MVP + Phase 2 + v3）
**承認状況**: レビュー待ち（品質評価93点A評価）
...
> [!tip] 💡 この機能が解決する問題
> アプリごとに新しいパスワードを作ると、覚えきれないし、使い回しによるセキュリティリスクも増えます。
> 既に信頼しているGitHub/Googleの認証を「借りる」ことで、安全かつ便利にログインできます。
...
>
> **解決**: ログアウトすると、このブラウザでの認証情報（→用語集「トークン」）が完全に消去されます。
...
>
> ⚠️ この機能は**Phase 2で実装予定**です。現在は利用できません。
...
> [!tip] 💡 この機能が解決する問題
>
> **困っていたこと**: サービス利用者の管理（→用語集「管理者」）が必要だが、方法がない
>
