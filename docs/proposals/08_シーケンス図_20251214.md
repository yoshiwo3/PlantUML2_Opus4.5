...
**作成日**: 2025-12-14（統合版）
**最終更新**: 2025-12-22（UC 5-11 学習コンテンツ登録追加）
**初版作成日**: 2025-11-30
...
13. [学習コンテンツ登録（UC 5-11）](#13-学習コンテンツ登録uc-5-11)
14. [技術仕様](#14-技術仕様)
...
## 13. 学習コンテンツ登録（UC 5-11）

**対象ユースケース**: UC 5-11 学習コンテンツを登録する
**対象機能**: F-ADM-11（学習コンテンツ登録）
**優先度**: Phase 2（AI機能高度化・学習支援）
**技術仕様**: TD-007（Embedding: OpenAI直接接続）
**設計パターン**: DP-001（レジリエンス）

開発者（管理者権限）が学習コンテンツ（ヘルプドキュメント）をシステムに登録するフローを表現します。登録されたコンテンツはF-DGM-10（学習コンテンツ検索）で利用可能になります。

### 13.1. 学習コンテンツ登録フロー

#### シーケンス図

![学習コンテンツ登録フロー](./diagrams/08_sequence/PlantUML_Studio_Sequence_LearningContentRegister.svg)
...
| フェーズ | 処理 | 説明 |
|---------|------|------|
| 登録フォーム表示 | 管理画面アクセス | 開発者が学習コンテンツ管理→新規登録をクリック |
| ファイルバリデーション | クライアント側チェック | ファイル形式（MD/PDF）とサイズ（≤10MB）を検証 |
| 登録実行 | サーバー処理 | 認証確認→タイトル重複チェック→コンテンツ登録 |
| チャンク分割 | テキスト処理 | 512トークン単位で分割、50トークンオーバーラップ |
| Embedding生成 | OpenAI API | バッチ処理でベクトル生成（text-embedding-3-small） |
| DB保存 | トランザクション | learning_contents, content_chunks, content_vectors同時保存 |
...
| 401 | UNAUTHORIZED | 未ログイン | 「ログインしてください」 |
| 403 | DEVELOPER_REQUIRED | 開発者権限なし | 「開発者権限が必要です」 |
| 409 | DUPLICATE_TITLE | タイトル重複 | 「同じタイトルのコンテンツが存在します」 |
| 422 | FILE_TOO_LARGE | ファイルサイズ超過 | 「ファイルサイズは10MB以下にしてください」 |
| 422 | INVALID_FORMAT | 非対応形式 | 「Markdown(.md)またはPDF(.pdf)のみ対応」 |
| 503 | EMBEDDING_FAILED | OpenAI API障害 | 「ベクトル生成に失敗しました」 |
...
#### 学習コンテンツ登録API（UC 5-11）

| エンドポイント | メソッド | 説明 | 認証 | 権限 |
|--------------|:-------:|------|:----:|:----:|
| `/api/admin/learning-content` | POST | 学習コンテンツ登録 | ✅ | developer |
...
interface RegisterLearningContentRequest {
  title: string;                                    // コンテンツタイトル（1〜200文字）
  category: string;                                 // カテゴリ（1〜50文字）
  content_type: 'puml' | 'excalidraw' | 'general';  // 対象図表タイプ
  file: File;                                       // Markdown/PDFファイル（最大10MB）
  tags?: string[];                                  // タグ一覧（任意）
}

interface RegisterLearningContentResponse {
  content_id: string;   // コンテンツID（UUID）
  chunk_count: number;  // 生成されたチャンク数
  token_count: number;  // 使用トークン数
}
```
...
type ErrorCode =
  ...
  // 学習コンテンツ登録（UC 5-11）
  | 'DEVELOPER_REQUIRED'       // 403: 開発者権限が必要
  | 'DUPLICATE_TITLE'          // 409: タイトル重複
  | 'FILE_TOO_LARGE'           // 422: ファイルサイズ超過（>10MB）
  | 'INVALID_FORMAT'           // 422: 非対応形式（MD/PDF以外）
  | 'EMBEDDING_FAILED';        // 503: OpenAI Embedding API障害
```
...
| 2025-12-21 | 2.7 | §8 AIチャット（UC 4-2）追加、§9 学習コンテンツ検索（UC 3-10）追加 |
| 2025-12-21 | 2.8 | §10 ユーザー管理（UC 5-1）追加、Supabase Admin API統合 |
| 2025-12-22 | 2.9 | §11 LLMモデル登録（UC 5-2）追加、§12 LLM使用量監視（UC 5-7）追加 |
| 2025-12-22 | 3.0 | **§13 学習コンテンツ登録（UC 5-11）追加、Phase 2シーケンス図完了（14/14）**、§14技術仕様へ繰り下げ |
