...
**関連ユースケース**: UC 3-10 学習コンテンツを検索する, UC 3-11 学習コンテンツを確認する

**フェーズ**: Phase 2（AI機能高度化・学習支援）

**前提条件**: 認証済みユーザー（無料会員も利用可能）

### 概要

学習コンテンツフローは、エンドユーザーがPlantUML構文や設計ベストプラクティスを学習するための機能群を提供する。以下の2つのサブフローで構成される：

| サブフロー | 説明 | 利用シーン |
|-----------|------|-----------|
| 3.10.1 エディタ内ヘルプフロー | エディタ操作中に即座にヘルプを参照 | 図表作成中の疑問解決 |
| 3.10.2 学習画面フロー | 体系的な学習、チュートリアル | 集中的な学習セッション |
...
**目的**: エディタで作業中に、選択コードや質問に基づいてリアルタイムでヘルプを提供する
...
1. **起動方法の複合対応**: ボタン、ショートカット、コンテキストメニューの3種類
2. **コンテキスト活用**: テキスト選択時は選択コードをクエリとコンテキストの両方に活用
3. **RAG検索**: Hybrid Search（ベクトル + 全文検索）で高精度な検索
4. **インラインアクション**: コピー、挿入、参照元表示をパネル内で完結

### 3.10.2 学習画面フロー

**目的**: 体系的な学習環境を提供し、ユーザーの学習進捗を管理する
...
1. **4つの主要機能**: カテゴリブラウズ、RAG検索、チュートリアル、お気に入り
2. **2階層カテゴリ**: 図表タイプ別 × 目的別で体系的に整理
3. **インタラクティブチュートリアル**: 説明→編集→確認のサイクルで実践的学習
4. **進捗管理**: 完了コンテンツ、進捗率、バッジで学習モチベーション維持

### 確定済み要件（17項目）

プランニング時に確定した要件を以下に整理する：
...
| エラー種別 | 原因 | 対応 |
|-----------|------|------|
| 検索結果なし | 類似コンテンツが閾値未満 | 「関連コンテンツが見つかりませんでした」+ 関連カテゴリへの誘導 |
| Embedding API エラー | OpenAI接続障害 | 「検索機能を一時的に利用できません」+ フォールバック（全文検索のみ） |
| LLM API エラー | OpenRouter接続障害 | 「AI回答生成に失敗しました」+ 検索結果のみ表示 |
| コンテンツ外質問 | PlantUML/設計と無関係 | 「この質問は学習コンテンツの範囲外です」+ 明確に対象外と表示 |
