...
**調査の優先順位**: § 3（既知の制限）→ Context7 → GitHub Issues → 公式ドキュメント
...
新たに発見したPlantUMLの制限（仕様・バグ起因、回避策あり）は、**問題解決直後**に § 3「その他の制限」テーブルに追加する。
...
| # | 禁止事項 | 詳細 |
|:-:|---------|:----:|
| 1 | **if/fork内でスイムレーン遷移するコードを書く** | § 3 |
| 2 | **SVGのXMLテキストを見て視覚確認したと判断する** | § 4.1 |
| 3 | **ソース+PNG対比確認をスキップする** | § 4.3 |
| 4 | **Context7確認なしでPlantUMLコードを作成する** | § 1.2 |
| 5 | **レビューログ未更新でPublishする** | § 4.4 |
| 6 | **レビュー後に.pumlを修正してPublishする** | § 5 |
...
| 日付 | バージョン | 変更内容 |
|------|-----------|---------|
| 2025-12-07 | 2.4 | **重複削減完了**: 全5Phase完了。706行→570行（136行削減、19%削減） |
| 2025-12-07 | 2.3 | **重複削減Phase 4**: § 1 必須プロセスを大幅簡潔化（60行→20行）、詳細は各セクション参照に |
| 2025-12-07 | 2.2 | **重複削減Phase 3**: § 2 禁止事項を簡潔化、「理由」→「詳細」参照に変更 |
