...
> **⚠️ v5.3改訂**: 評価・採点を3フェーズに分離。各フェーズで**90点以上**が合格ライン。
...
│           ↓                                                     │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ ① Phase 1-B: 視覚品質評価（§1.3参照）                   │    │
│  │    4. PNG + レビューログ生成（-Review）                  │    │
│  │    5. 5パスレビュー（§4.2参照）- PNG単独確認             │    │
│  │    6. 評価・採点（視覚品質観点）                         │    │
│  │    - 90点未満 → コード修正 → 再評価                     │    │
│  │    ※確証バイアス低減: ソースを見ずにPNGのみで評価       │    │
│  └─────────────────────────────────────────────────────────┘    │
│           ↓ 90点以上                                            │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ ② Phase 1-A: コード品質評価（§1.3A参照）                │    │
│  │    7. ソース+PNG対比確認（§4.3参照）                     │    │
│  │    8. 評価・採点（コード品質観点）                       │    │
│  │    - 90点未満 → コード修正 → Phase 1-B に戻る           │    │
│  └─────────────────────────────────────────────────────────┘    │
│           ↓ 90点以上                                            │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ Phase 2: Publish                                        │    │
...
│           ↓                                                     │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ ③ Phase 2: ドキュメント統合評価（§1.4参照）             │    │
│  │    - 統合版Markdownに追記                                │    │
│  │    - ドキュメント品質を評価・採点                        │    │
│  │    - 90点未満 → ドキュメント修正 → 再評価               │    │
│  └─────────────────────────────────────────────────────────┘    │
│           ↓ 90点以上                                            │
│  完了 → active_context.md更新 → SERENA Memory保存               │
└─────────────────────────────────────────────────────────────────┘
```

#### 3フェーズ評価体系サマリ

| フェーズ | 評価タイミング | 評価観点 | 合格ライン | 参照 |
|:-------:|---------------|---------|:----------:|:----:|
| **① Phase 1-B** | PNG生成後（5パスレビュー） | 視覚品質・アクティブバー・接続線 | **90点** | §1.3 |
| **② Phase 1-A** | 視覚評価合格後 | 構文・クラス図整合性・DP適用 | **90点** | §1.3A |
| **③ Phase 2** | 統合版追記後 | ドキュメント品質・API仕様・参照 | **90点** | §1.4 |

> **⚠️ v5.4改訂**: 確証バイアス低減のため評価順序を逆転。視覚評価（1-B）を先に実施し、ソースを見ずにPNGのみで評価することで、コード知識による先入観を排除する。
...
### 1.3 Phase 1-B: 視覚品質評価（PNG生成後 - 確証バイアス低減のため先行実施）

> **⚠️ v5.4改訂**: 確証バイアス低減のため、**ソースコードを見ずに**PNGのみで視覚品質を評価する。

```
コード作成完了
    ↓
┌──────────────────────────────────────────────────────────┐
│ 【Phase 1-B: 視覚品質評価ループ】（確証バイアス低減）     │
│                                                          │
│  1. PNG生成: `-Review`でPNG + レビューログ生成           │
│         ↓                                                │
│  2. 5パスレビュー: § 4.2 参照（PNGのみを確認）           │
│         ↓                                                │
│  3. 評価・採点（6カテゴリ、§1.3.1参照）                  │
│         ↓                                                │
│  4. 90点未満 or 視覚的問題あり？                         │
│         ↓ はい                                           │
│  5. 問題点を特定・修正 → コード修正 → 1 に戻る           │
│                                                          │
│  ※この時点ではソースコードを参照しない（先入観排除）    │
└──────────────────────────────────────────────────────────┘
    ↓ 90点以上 & 視覚的問題なし
Phase 1-A: コード品質評価へ進む（§1.3A参照）
```

#### 1.3.1 Phase 1-B 評価・採点基準（視覚品質）

> **⚠️ 採点は「厳しく」実施すること。** PNGを視覚的に確認し、問題があれば確実に減点する。
> **⚠️ 確証バイアス低減**: この評価中はソースコードを参照しない。PNGのみで判断する。

**6カテゴリで90点満点評価。90点以上を合格ラインとする。**

| カテゴリ | 配点 | 評価内容 |
|---------|:----:|---------|
| LL準拠 | 25 | LL-001〜LL-027への準拠（特にLL-001: else分岐状態継承） |
| アクティブバー | 20 | activate/deactivate対応、バーの開始・終了位置 |
| 接続線 | 20 | 全メッセージが正しく描画、途切れ・孤立なし |
| レイアウト | 10 | 参加者配置、note位置、可読性 |
| 既存パターン一貫性 | 10 | skinparam、色分け、命名規則の統一 |
| スタイル | 5 | フォント、線種、余白の適切さ |
| **合計** | **90** | **90点以上で合格** |
...
| # | 視覚的問題 | 例 |
|:-:|---------|-----|
| 1 | アクティブバーの欠落 | メッセージ送信元にバーがない |
| 2 | アクティブバーの過剰延長 | deactivate漏れでバーが下まで伸びている |
| 3 | 接続線の途切れ | alt分岐内でメッセージが消えている |
| 4 | 参加者の孤立 | 一度も呼び出されない参加者がいる |

#### 1.3.3 Phase 1-B 改善手順（5ステップ）

1. **PNG生成**: `-Review`でPNG + レビューログ生成
2. **5パスレビュー**: § 4.2 に従い構造→接続→内容→スタイル→設計パターン（**PNGのみ確認**）
3. **採点実施**: 上記6カテゴリで採点（テーブル形式で記録）
4. **視覚的問題確認**: § 1.3.2 に該当するか判定
5. **90点以上 & 視覚的問題なし** → Phase 1-A（§1.3A）へ

> **⚠️ 確証バイアス低減**: Phase 1-Bではソースコードを参照**しない**。PNGのみで視覚品質を評価する。

#### 1.3.4 Phase 1-B 採点結果の記録
...
| カテゴリ | 配点 | 得点 | 減点理由 |
|---------|:----:|:----:|---------|
| LL準拠 | 25 | 20 | LL-001違反（2箇所: else分岐でactivate漏れ） |
| アクティブバー | 20 | 18 | deactivate漏れ（1箇所） |
| ...      | ... | ... | ... |
| **合計** | **90** | **85** | - |
...
**修正内容**:
1. Line 78: else分岐でUserServiceをactivate
2. Line 92: OpenRouterClientのdeactivate追加
...
### 1.3A Phase 1-A: コード品質評価（視覚評価合格後 - ソース+PNG対比確認）

> **⚠️ 重要**: Phase 1-B（視覚評価）合格後、**ソースコードを読み**、PNGと対比しながらコード品質を評価・採点する。

```
Phase 1-B 合格（90点以上）
    ↓
┌──────────────────────────────────────────────────────────┐
│ 【Phase 1-A: コード品質評価ループ】                       │
│                                                          │
│  1. ソース+PNG対比確認: § 4.3 参照                       │
│         ↓                                                │
│  2. コードを読み、評価・採点（6カテゴリ、§1.3A.1参照）   │
│         ↓                                                │
│  3. 90点未満 or 重大問題あり？                           │
│         ↓ はい                                           │
│  4. 問題点を特定・修正 → コード修正 → Phase 1-Bに戻る    │
│                                                          │
└──────────────────────────────────────────────────────────┘
    ↓ 90点以上 & 重大問題なし
Phase 2: SVG Publish → ドキュメント統合評価へ（§1.4参照）
```

#### 1.3A.1 Phase 1-A 評価・採点基準（コード品質）
...
| カテゴリ | 配点 | 評価内容 |
|---------|:----:|---------|
| 構文・構造 | 20 | alt/else/end対応、activate/deactivate対応、skinparam適用 |
| クラス図整合性 | 20 | メソッド名・引数がクラス図と一致、勝手な追加なし |
| 設計パターン | 20 | DP-001〜DP-006の適用（タイムアウト/リトライ/監査ログ等） |
...
#### 1.3A.3 Phase 1-A 改善手順（5ステップ）

1. **対比確認**: § 4.3 に従いソースとPNGを対比確認
2. **コード読解**: .puml を読み、各評価カテゴリをチェック
3. **採点実施**: 上記6カテゴリで採点（テーブル形式で記録）
4. **重大問題確認**: § 1.3A.2 に該当するか判定
5. **90点以上 & 重大問題なし** → SVG Publish → Phase 2（§1.4）へ

> **注意**: Phase 1-Aで不合格の場合、コード修正後は**Phase 1-Bから再評価**する。

#### 1.3A.4 Phase 1-A 採点結果の記録
...
| カテゴリ | 配点 | 得点 | 減点理由 |
|---------|:----:|:----:|---------|
| 構文・構造 | 20 | 18 | activate/deactivate不整合（2箇所） |
| クラス図整合性 | 20 | 15 | registerModel()の引数が不一致 |
| ...      | ... | ... | ... |
| **合計** | **90** | **83** | - |
...
**修正内容**:
1. Line 56-57: else分岐のactivate/deactivate修正
2. Line 66: registerModel(dto)に修正
...
| カテゴリ | 配点 | 評価内容 |
|---------|:----:|---------|
| セクション構成 | 20 | 既存セクションとの整合性、番号連続性、目次更新 |
| API仕様 | 20 | Request/Response型定義、エンドポイント仕様、エラーコード表 |
| 業務フロー整合性 | 15 | 業務フロー図との一致、対応UC明記 |
| 既存パターン一貫性 | 15 | 命名規則、フォーマット、参照スタイル |
| 参照・リンク | 15 | SVGパス、セクション参照、更新履歴 |
...
| 10 | 6カテゴリで採点し、結果を記録した | work_sheet.mdに記録 |
...
| 日付 | バージョン | 変更内容 |
|------|-----------|---------|
| 2025-12-22 | 5.4 | **評価順序逆転（確証バイアス低減）**: ① Phase 1-B（視覚品質評価）を先に、Phase 1-A（コード品質評価）を後に実行する順序に変更、② §1.3を視覚品質評価、§1.3Aをコード品質評価に再構成、③ 全体フロー図・サマリテーブル・セクション内容を更新、④ 「ソースを見ずにPNGのみで評価」の原則を明記。視覚確認時にコード知識による先入観を排除し、確証バイアスを低減する。 |
| 2025-12-22 | 5.3 | **3フェーズ評価体系導入**: ① Phase 1-A（コード品質）、Phase 1-B（視覚品質）、Phase 2（ドキュメント統合）の3段階評価を正式化、② 各フェーズで90点以上を合格ラインに設定、③ 6カテゴリ90点満点に統一（旧8カテゴリ95点から変更） |
