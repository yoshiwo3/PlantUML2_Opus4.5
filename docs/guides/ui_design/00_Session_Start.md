...
> **🚨 CS-005違反防止**: 「7. ユーザーに提示・確認待ち」は**絶対にスキップしてはならない**。詳細: `UI_Design_Knowledge_Base.md` CS-005参照。
>
> **🚨 CS-007違反防止（v1.11追加）**: イテレーションループは**2箇所**で発生する:
> - **イテレーションループ1（Step 5）**: 5パスレビュー後、90点未満ならユーザー提示前に修正
> - **イテレーションループ2（Step 9）**: ユーザー編集後のAI厳格評価で90点未満なら修正
>
> **両方のイテレーションループで知見ベースを参照**すること。詳細: `UI_Design_Knowledge_Base.md` CS-007参照。
...
- [ ] アクセシビリティ考慮（憲法 §8）
- [ ] **5パスレビュー（AI自己評価）**（憲法 §4.1）← ワイヤーフレーム作成直後に実施
- [ ] **[90点未満時] 知見ベース参照・修正 → ワイヤーフレーム修正に戻る（イテレーションループ1）**
- [ ] **⚠️ `00_wireframe_index.md` 即時更新（必須）** ← 90点以上達成後に実行

> **🚨 重要（v1.11追加）**: 5パスレビュー後、**90点未満の場合はイテレーションループ1を実行**すること。知見ベース（CS-001〜006）を参照し、問題を修正してから再度5パスレビューを実施する。**90点以上になるまでユーザーに提示してはならない**。

> **🚨 重要（v1.7追加）**: ワイヤーフレームファイル（`.excalidraw`）を作成し**90点以上を達成したら、即座に** `00_wireframe_index.md` を更新すること。
...
## 6. 引継ぎファイル作成チェックリスト（CS-004）

セッション終了時に引継ぎファイルを作成する際は、以下を確認すること。

### 6.1 Evidence管理パターンの明示（必須）

引継ぎファイルには、次セッションのEvidence管理方式を**必ず明記**すること。
...
> **⚠️ 重要**: どちらのパターンかを明示しないと、新セッションで混乱が生じる。

### 6.2 暗黙知の明示化チェック

| # | 確認項目 | 質問形式 | 例 |
|:-:|---------|---------|-----|
| 1 | **Evidence管理** | 継続利用か新規作成か？ | パターンA/Bを明記 |
| 2 | **継続リソース** | 新セッションで再利用すべきファイル/ディレクトリは？ | 既存ワイヤーフレーム、設定ファイル |
| 3 | **禁止事項** | 新セッションでやってはいけないことは？ | 特定ファイルの上書き禁止 |
| 4 | **前提条件** | 当然だと思っているが明示すべきことは？ | 作業プロセスのバージョン |
| 5 | **コンテキスト依存** | このセッション固有の設定/状態は？ | 知見ベースのバージョン |

### 6.3 引継ぎファイル必須記載項目

- [ ] 新セッション開始時に読むべきファイル一覧（順序付き）
- [ ] **Evidence管理パターン（A: 継続利用 / B: 新規作成）**
- [ ] 完了した作業の概要
- [ ] 次のステップ（推奨タスク）
- [ ] 重要な技術決定・知見のサマリー

> **教訓（CS-004）**: 「当然のこと」こそ明示的に書く。暗黙知は新しいコンテキスト（新セッション）では通用しない。
...
| 日付 | 内容 |
|------|------|
| 2025-12-28 | **v1.11**: **イテレーションループ2箇所化（CS-007）**: ①5パスレビュー後のイテレーションループ1追加（Step 5）、②ユーザー編集後のイテレーションループ2明確化（Step 9）、③Step 5チェックリストにイテレーションループ1追加、④「90点以上になるまでユーザーに提示禁止」ルール明文化 |
| 2025-12-28 | v1.10: Step 4 Todoテンプレート修正（憲法§1.4.1準拠）: 5パスレビューをワイヤーフレーム作成直後に配置、知見ベース参照追加 |
| 2025-12-28 | v1.9: CS-005違反防止警告追加（ユーザー確認スキップ禁止）|
| 2025-12-28 | v1.8: §6「引継ぎファイル作成チェックリスト（CS-004）」追加（暗黙知の明示化チェック、必須記載項目）|
| 2025-12-28 | v1.7: Step 5に`00_wireframe_index.md`**即時更新ルール**追加（ファイル作成直後に実行、Step 8まで待たない）|
