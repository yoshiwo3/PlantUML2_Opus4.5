...
> **重要**: 最新のテンプレートは `00_Session_Start.md` Step 4を参照。CS-005/CS-006/CS-007違反防止の詳細も記載。
> **CS-007**: イテレーションループは**2箇所**で発生（Step 5: 5パスレビュー後、Step 9: ユーザー編集後）。**90点以上になるまでユーザーに提示禁止**。
...
| ドキュメント | パス | 目的 |
|-------------|------|------|
| **UI設計図表開発憲法** | `docs/guides/ui_design/UI_Design_Constitution.md` | v3.2、禁止事項10項目、3フェーズ評価、命名規則 |
...
| 日付 | 内容 |
|------|------|
| 2025-12-28 | **v1.2**: **イテレーションループ2箇所化（CS-007）**: ①Step 5（5パスレビュー後）、②Step 9（ユーザー編集後）を明確化。「90点以上になるまでユーザーに提示禁止」ルール追加 |
| 2025-12-28 | v1.1: §0.4 Todoテンプレートを`00_Session_Start.md`と同期（5パスレビュー順序、知見ベース参照、CS-005/CS-006対応）|
