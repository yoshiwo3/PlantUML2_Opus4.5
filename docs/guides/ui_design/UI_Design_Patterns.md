...
### Excalidrawパターン（EP）適用条件

| パターン | 適用条件 | 適用しない場合 |
|---------|---------|---------------|
| **EP-001** 画面フレーム構造 | 全画面で必須 | - |
| **EP-002** ヘッダー | グローバルナビが必要な画面 | モーダル、ダイアログ |
| **EP-003** フォーム入力 | ユーザー入力がある画面 | 一覧表示のみの画面 |
| **EP-004** ボタン | アクションが必要な画面 | 情報表示のみの画面 |
| **EP-005** カード/リスト | 複数項目を表示する画面 | 単一情報の詳細画面 |
| **EP-006** モーダル/ダイアログ | 確認・入力ポップアップ | メイン画面 |
| **EP-007** エラー/フィードバック | 入力検証がある画面 | 静的コンテンツ |

### PlantUMLパターン（SP）適用条件

| パターン | 適用条件 | 適用しない場合 |
|---------|---------|---------------|
| **SP-001** skinparam設定 | 全遷移図で必須 | - |
| **SP-002** 状態定義 | 画面ノード作成時 | - |
| **SP-003** 複合状態 | 5画面以上のグループ | 小規模な遷移図 |
| **SP-004** 遷移（矢印） | 画面間の移動がある場合 | - |
| **SP-005** 開始点/終了点 | 全遷移図で必須 | - |
