...
| [ケーススタディ](#ケーススタディ) | 具体的事例と教訓（CS-001〜） |
| [メタ知見](#メタ知見meta-level-insights) | プロセス設計の指針（ML-001〜） |
| [Todo作成作法](#todo作成作法todoテンプレート規約) | **Todoテンプレート規約**（マスターソース、必須ルール） |
...
## Todo作成作法（Todoテンプレート規約）

### 概要

本セクションは、UI設計図表作成時のTodoリスト作成規約を定義する。Todoテンプレートは協調作成プロセスの根幹であり、正確な遵守が必須。

### マスターソース

| 項目 | 内容 |
|------|------|
| **正式定義** | `00_Session_Start.md` Step 4 |
| **現行バージョン** | v1.11（14項目、2イテレーションループ） |
| **同期対象** | `01_Reference_Guide.md` §0.4 |

> **原則**: Todoテンプレートを変更する場合は、マスターソース（00_Session_Start.md）を更新し、同期対象に反映すること。
...
| # | ルール | 違反時の影響 | 関連CS |
|:-:|--------|-------------|:------:|
| 1 | **2イテレーションループ必須** | 低品質成果物がユーザーに提示される | CS-007 |
| 2 | **Step 7（ユーザー確認）スキップ禁止** | 協調作成プロセスの根本違反 | CS-005 |
| 3 | **5パスレビューはStep 4で実施** | 品質ゲート迂回 | CS-006 |
| 4 | **00_wireframe_index.md即時更新（Step 6）** | 一覧管理の破綻 | CS-003 |
...
Step 3: ワイヤーフレーム作成
    ↓
Step 4: 5パスレビュー（AI自己評価）
    ↓
Step 5: 【イテレーションループ1】← 90点未満時
    │   └─→ 知見ベース参照 → Step 3に戻る
    ↓
Step 6: 00_wireframe_index.md即時更新
    ↓
Step 7: ⚠️ ユーザーに提示・確認待ち（必須）
    ↓
Step 8: AI厳格評価・採点
    ↓
Step 9: 【イテレーションループ2】← 90点未満時
    │   └─→ 知見ベース参照 → Step 3に戻る
    ↓
Step 10〜14: 成果物保存・記録
```
...
イテレーションループ発動時は、以下のケーススタディを確認すること：

| ID | 確認内容 | 典型的な問題 |
|:--:|---------|-------------|
| CS-003 | 即時更新ルール | 一覧更新漏れ |
| CS-005 | ユーザー確認必須 | Step 7スキップ |
| CS-006 | テンプレート同期 | 憲法との不整合 |
| CS-007 | 2イテレーション必須 | ループ欠落 |
| CS-008 | 座標正規化 | ユーザー編集後の整合性 |

### テンプレート変更時の手順

1. **マスターソース更新**: `00_Session_Start.md` Step 4を編集
2. **同期対象更新**: `01_Reference_Guide.md` §0.4を同期
3. **知見ベース更新**: 本セクションの「正式Todoテンプレート」を更新
4. **バージョン番号更新**: 00_Session_Start.md更新履歴にバージョン追記
5. **変更理由を記録**: CSまたはML知見として記録

> **教訓（CS-006）**: 同じ情報を複数ドキュメントに記載する場合、**マスターソースを定義**し、変更時に同期を確認すること。
...
| 日付 | Ver | 内容 |
|------|:---:|------|
| 2025-12-28 | **1.15** | **Todo作成作法セクション追加**: Todoテンプレート規約（マスターソース、14項目、2イテレーションループ、必須ルール）を体系的に文書化 |
