...
| 追加知見 | LL-021〜LL-027 | ネストalt、初期activate、段階的リファクタリング、activate漏れ防止、クラス図整合性、設計パターン適用トリガー |
...
### LL-026: シーケンス図作成前のクラス図整合性確認

**追加日**: 2025-12-21
**出典**: UC 4-2 AIチャット作成時のUltrathink分析

シーケンス図で使用するServiceメソッドがクラス図に存在することを事前確認する。
...
**解決策**:
1. シーケンス図で使用予定のServiceメソッドをリストアップ
2. クラス図で存在確認
3. 不足していれば**クラス図を先に更新**
4. シーケンス図を作成

**クラス図 v1.8 での更新内容**:
- `AIService.chat(message, context, chatMode, messages[])` メソッド追加
- 値オブジェクト追加: `ChatContext`, `ChatResponse`, `Suggestion`, `Message`
- 列挙型追加: `ChatMode`, `ChatPhase`, `SuggestionType`, `MessageRole`

**チェックリスト**:
- [ ] シーケンス図で呼び出すService/Repositoryメソッドをリストアップ
- [ ] クラス図で各メソッドの存在を確認
- [ ] メソッドの引数・戻り値がシーケンス図と一致するか確認
- [ ] 不一致があれば、クラス図を先に更新

**関連**: LL-025（ネストalt）、シーケンス図作成ガイド §3.4

### LL-027: 設計パターン知識の適用トリガー欠如

**追加日**: 2025-12-21
**出典**: UC 3-10 学習コンテンツ検索作成後のUltrathink根本原因分析

**問題**: デバウンス、キャッシュ、リトライ、503エラー等の「設計パターンの知識」はあったが、プロセスに「適用トリガー」がなかったため適用しなかった。

**これはLL-025の拡張パターン**:
- LL-025: 「LL-001の知識はあったが適用しなかった」（PlantUML構文）
- LL-027: 「設計パターンの知識はあったが適用しなかった」（アーキテクチャ設計）

**根本原因**:
1. プロセスに設計パターン検証ステップがなかった
2. 4パスレビューがPlantUML構文に偏重していた
3. 参照パターン（§7, §8）を無批判にコピーした
4. UC固有分析（リアルタイム検索 → デバウンス必要）を行わなかった

**解決策**:
1. **5パスレビュー化**: Pass 5「設計パターン検証」を追加
2. **UC固有分析**: 作成前に設計パターン要否を分析
3. **設計パターンチェックリスト**: DP-001〜DP-006を確認

**チェックリスト（Pass 5）**:
- [ ] 外部API呼び出しにタイムアウト・リトライ・503があるか？（DP-001）
- [ ] リアルタイム入力にデバウンスがあるか？（DP-002）
- [ ] 繰り返し呼び出しにキャッシュがあるか？（DP-003）
- [ ] 高頻度APIにレート制限があるか？（DP-004）
- [ ] 管理操作に監査ログがあるか？（DP-005）

**関連ドキュメント**:
- `docs/guides/sequence_diagram/Design_Pattern_Checklist.md`
- `docs/guides/PlantUML_Development_Constitution.md` §4.2（5パスレビュー）
