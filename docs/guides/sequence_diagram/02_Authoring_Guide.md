...
> **⚠️ 重要**: 以下の整合性確認をコード作成**前**に実施すること。確認せずに作成したコードは修正が必要になる。

- [ ] **クラス図確認**: クラス図v1.8を確認し、使用するService/Repositoryの**メソッド名・引数が完全一致**するか検証した
- [ ] **業務フロー確認**: 業務フロー図の該当セクションを読み、**必須機能を全てリストアップ**した
- [ ] **UC定義確認**: 機能一覧表の該当UC番号を確認し、**全操作がカバーされている**か検証した
...
|-------------|---------------------|:----:|
| UserService.listUsers(filter?) | listUsers(filter?) | ✅ |
| UserService.getUser(userId) | getUser(userId) | ✅ |
```

→ **不一致がある場合、クラス図の定義に合わせること**（勝手に引数を追加しない）
...
> **参照**: PlantUML開発憲法 v5.1 §1.3

- [ ] PNG生成（`-Review`）を実施した
- [ ] 5パスレビューを実施した
- [ ] **8カテゴリで採点**し、結果をwork_sheet.mdに記録した
- [ ] **90点以上**かつ**重大問題なし**を確認した
- [ ] 90点未満または重大問題がある場合、修正して再採点した

**採点カテゴリ（95点満点、90点以上合格）**:
...
### ドキュメント統合・採点（SVG Publish後必須）

> **参照**: PlantUML開発憲法 v5.1 §1.4

- [ ] 正式版Markdownドキュメントに統合した
- [ ] **8カテゴリで採点**し、結果をwork_sheet.mdに記録した
- [ ] **90点以上**を確認した
- [ ] 90点未満の場合、修正して再採点した

**ドキュメント統合採点カテゴリ（95点満点、90点以上合格）**:
...
- [ ] PlantUML開発憲法v5.2に従いPNG生成→視覚的レビュー→採点→改善ループを完了した
- [ ] **90点以上**の採点結果を確認した
- [ ] SVGを正式版として保存した
- [ ] ドキュメント統合後、**90点以上**の採点結果を確認した

### 知見反映（セッション終了前必須）

> **参照**: PlantUML開発憲法 v5.2 §1.7

- [ ] LL違反があった → `Activation_Bar_Knowledge_Base.md` に追加（LL-XXX or CS-XXX）
...
| `PlantUML_Development_Constitution.md` | PlantUML開発憲法（v5.2: 5パスレビュー、知見反映§1.7） |
...
| 2025-12-21 | §7「設計確認」強化（クラス図整合性確認テーブル必須化）、「改善ループ・採点」セクション追加（憲法v5.0準拠） |
| 2025-12-21 | §7「知見反映」セクション追加（憲法v5.2 §1.7準拠）、参照ドキュメント更新 |
