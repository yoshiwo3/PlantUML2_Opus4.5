...
**問題**: 同一情報が複数ファイルに存在する場合、更新漏れが発生しやすい

**発生事例**:
| セッション | 問題ID | 影響ファイル |
|:----------:|--------|-------------|
| 8 | CS-003 | `00_Session_Start.md` + `00_wireframe_index.md` |
| 11 | CS-006 | `00_Session_Start.md` + `01_Reference_Guide.md` |
| 11 | CS-007 | `00_Session_Start.md` + `01_Reference_Guide.md` + `Knowledge_Base.md` |

**対策案（未実施）**:
1. **マスターソース明示**: 各情報について「どのファイルが正」かを明示
2. **参照リンク方式**: 重複を避け、参照先を示すリンクのみ記載
3. **同期チェックリスト**: 更新時に関連ファイル一覧を確認するルール化

**優先度**: 中（現状は都度修正で対応可能だが、根本解決が望ましい）
