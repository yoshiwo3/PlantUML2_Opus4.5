...
## 3.5 プロジェクト管理フロー追加（同セッション継続）

**作成日**: 2025-12-02
**対応UC**: 2-1, 2-2, 2-3

### ユーザー決定事項
- フロー構成: 統合フロー（1つ）で3UC全てを表現
- 削除確認: 確認ダイアログ必須（配下図表の警告表示）
- 作成属性: 名前のみ

### 作成内容
- PlantUMLアクティビティ図（switch/case構文）
- スイムレーン: エンドユーザー, Frontend Service, Supabase
...
**ユースケースカバレッジ**: 11/23 (48%)

## 3.5 視認性改善（追記）

### 問題点
- `stop`がrepeat whileを壊す（48/100評価）
- 5階層ネスト、10回スイムレーン遷移
- 163行（長すぎ）
...
残り5フロー（MVP 3 + Phase2 2）:
- MVP: 3.6 保存・エクスポート, 3.7 バージョン管理, 3.8 図表削除
